...
if __name__ == "__main__":
    ...
    soft_pose.p = gymapi.Vec3(0.02, 0.365-two_robot_offset, 0.02)
    ...
    dof_props_2["stiffness"].fill(500.0)
    dof_props_2["damping"].fill(100.0)
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            # elif t <= 7:     
            #     state = "get plan" 
                # state = "set up velocity mode2"
                # gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka2", "psm_yaw_joint"), 0.2)
                
            
            # else:
            #     gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka2", "psm_pitch_end_joint"), -0.1)

        if state == "pick up":
            rospy.loginfo("**Current state: " + state)
            before_servo_pos = 0.05
            gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka", "psm_main_insertion_joint"), before_servo_pos)
            gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka", "psm_tool_gripper1_joint"), 0.4)
            gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka", "psm_tool_gripper2_joint"), -0.3) 
        ...
        if state == "get plan":
            ...
            current_poses = deepcopy(gym.get_actor_rigid_body_states(envs[i], kuka_handles_2[i], gymapi.STATE_POS)[-3])
            print("***Current x, y, z: ", current_poses["pose"]["p"]["x"], current_poses["pose"]["p"]["y"], current_poses["pose"]["p"]["z"] )              
            
            # delta_x = 0.04   
            # delta_y = 0.03 
            # delta_z = 0.025  
            delta_x = np.random.uniform(low = -0.04, high = 0.04)   
            delta_y = np.random.uniform(low = 0.0, high = 0.03)
            delta_z = np.random.uniform(low = 0.0, high = 0.025) 
            ...
            cartesian_pose.position.x = -current_poses["pose"]["p"]["x"] + delta_x
            cartesian_pose.position.y = -current_poses["pose"]["p"]["y"] + delta_y
            cartesian_pose.position.z = current_poses["pose"]["p"]["z"] - ROBOT_Z_OFFSET + delta_z
        ...
        if state == "move to goal":
            dof_states = gym.get_actor_dof_states(envs[0], kuka_handles_2[0], gymapi.STATE_POS)['pos']
            plan_traj_with_gripper = [plan+[0.4,-0.3] for plan in plan_traj]
            ...
            if traj_index == len(plan_traj):
                ...
                final_poses = gym.get_actor_rigid_body_states(envs[i], kuka_handles_2[i], gymapi.STATE_POS)[-3]
                print("***Final x, y, z: ", final_poses["pose"]["p"]["x"], final_poses["pose"]["p"]["y"], final_poses["pose"]["p"]["z"] ) 
                delta_x = -(final_poses["pose"]["p"]["x"] - current_poses["pose"]["p"]["x"])
                delta_y = -(final_poses["pose"]["p"]["y"] - current_poses["pose"]["p"]["y"])
                delta_z = final_poses["pose"]["p"]["z"] - current_poses["pose"]["p"]["z"]
        ...
        if state == "get point cloud":
            ...
            final_point_clouds.append(list(point_cloud))     
        ...
        #     with open('/home/baothach/shape_servo_data/batch_3', 'wb') as handle:
    ...
    # data = {"point clouds": final_point_clouds, "velocities": final_desired_velocities}
    # data = {"point clouds": final_point_clouds, "positions": final_desired_positions, "saved intial state":saved_object_state}
    # with open('/home/baothach/shape_servo_data/batch_3', 'wb') as handle:
    #     pickle.dump(data, handle, protocol=pickle.HIGHEST_PROTOCOL)
