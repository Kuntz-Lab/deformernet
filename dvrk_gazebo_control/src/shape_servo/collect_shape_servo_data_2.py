...
if __name__ == "__main__":
    ...
    dof_props_2["stiffness"][8:].fill(0.001)
    dof_props_2["damping"][8:].fill(0.002)  
    ...
    final_feature_vectors_30 = []
    final_desired_velocities = []
    ...
    while (not gym.query_viewer_has_closed(viewer)) and (not all_done):
        ...
        if state == "set up velocity mode":
            ...
            gym.set_actor_dof_properties(envs[0], kuka_handles[0], dof_props)
            state = "get point cloud"
        
        if state == "get point cloud":
            rospy.loginfo("**Current state: " + state)
            
            # main_ins_pos = gym.get_joint_position(envs[0], gym.get_joint_handle(envs[0], "kuka", "psm_main_insertion_joint"))
            
            # if np.allclose(main_ins_pos, 0.24, atol=0.001):
            #     mode = "negative"
            # elif np.allclose(main_ins_pos, 0.225, atol=0.001):
            #     mode = "positive"
            ...
            if frame_count % 2 == 0:
                if first_time_step:
                    ...
                    main_ins_init_pos = gym.get_joint_position(envs[0], gym.get_joint_handle(envs[0], "kuka", "psm_main_insertion_joint"))
                    
                    gym.refresh_particle_state_tensor(sim)
                    particle_state_tensor = deepcopy(gymtorch.wrap_tensor(gym.acquire_particle_state_tensor(sim)))
                    particle_states = particle_state_tensor.numpy()[:, :3]  
                    final_point_clouds.append(list(particle_states))
                    final_desired_positions.append(0)
                # else:  
                #     main_ins_vel = gym.get_joint_velocity(envs[0], gym.get_joint_handle(envs[0], "kuka2", "psm_main_insertion_joint"))
                #     final_desired_velocities.append(main_ins_vel)  
                #     sample_count += 1                               
                ...
                if sample_count > 11:                
                    ...
                    particle_states = particle_state_tensor.numpy()[:, :3]     
                    final_point_clouds.append(list(particle_states))     
                    final_desired_positions.append(main_ins_pos  -  main_ins_init_pos)   
            ...
            frame_count += 1        
                ...
                # if first_time_step:
                #     first_time_step = False
                #     old_feature_vector_30 = feature_vector_30

                # else:
                #     final_feature_vectors_30.append(list(feature_vector_30-old_feature_vector_30))
                #     main_ins_vel = gym.get_joint_velocity(envs[0], gym.get_joint_handle(envs[0], "kuka2", "psm_main_insertion_joint"))
                #     final_desired_velocities.append(main_ins_vel)
                #     old_feature_vector_30 = feature_vector_30
    ...
    # with open('/home/baothach/shape_servo_data/data_for_training_end_to_end_new_task(2)', 'wb') as handle:
