...
def get_manipulability_measure(feature_vector, position):
    jacobian = (np.linalg.pinv(position) @ feature_vector)*10
    # print(jacobian.shape)
    return np.linalg.det(jacobian @ jacobian.T)
...
if __name__ == "__main__":
    ...
    # Get goal pc:
    with open('/home/baothach/shape_servo_data/multi_grasps/goal_1a', 'rb') as handle:
    ...
    model.load_state_dict(torch.load("/home/baothach/shape_servo_data/multi_grasps/weights/batch_1-epoch 12"))  
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            if count == 1:
                grasp_pose = torch.tensor([0,-0.375, 0.0333]).float().unsqueeze(0)
            elif count == 2:
                grasp_pose = torch.tensor([0.0385,-0.43, 0.0333]).float().unsqueeze(0)
            elif count == 3:
                grasp_pose = torch.tensor([-0.0385,-0.375, 0.0333]).float().unsqueeze(0)  
            count += 1              
            model.eval()
            desired_position = model(current_pc.unsqueeze(0), goal_pc.unsqueeze(0), grasp_pose)[0].detach().numpy()*(0.001)  
            ...
            # Feature vector 256
            feature_vector = model(current_pc.unsqueeze(0), goal_pc.unsqueeze(0), grasp_pose, get_feature_vector = True).detach().numpy()
            # print(feature_vector.shape)
            mani_measure = get_manipulability_measure(feature_vector, np.expand_dims(desired_position, axis=0))
            print("manipulation measure " +str(count-1) ,  mani_measure)
