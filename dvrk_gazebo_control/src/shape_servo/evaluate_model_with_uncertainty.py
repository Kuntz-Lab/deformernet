...
if __name__ == "__main__":
    ...
    # syrup_pose.p = gymapi.Vec3(0.0, 0.4, 0.0)
    ...
    state = "home"
    # goal = FPFH_135_vectors[5].reshape(1, -1)  # desired feature vector for shape servoing
    # goal = pca.transform(goal)    # size (1,30)
    ...
    model = Net()
    model.load_state_dict(torch.load("/home/baothach/shape_servo_data/weights/shape_servo_weights"))    
    ...
    while (not gym.query_viewer_has_closed(viewer)) and (not all_done):
        ...
        if state == "home" :   
            ...
            if t <= 1:
                gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka", "psm_main_insertion_joint"), 0.05)
                ...
                if np.allclose(main_ins_pos, 0.15, atol=0.003):
                    ...
                    # gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka2", "psm_main_insertion_joint"), 0.233)                    
        ...
        if state == "get point cloud initial":
            ...
            if frame_count_pc == 60:
                ...
                pcd_goal.points = open3d.utility.Vector3dVector(np.array(points_inital))            
                ros_cloud = orh.o3dpc_to_rospc(pcd_goal)
                goal_135 = np.array(VFH_client(ros_cloud)).reshape(1, -1)  # np array size (1,135)
                goal = pca.transform(goal_135) # np array size (1,30)  
                state = "set up velocity mode"
                # state = "move to initial position"
                frame_count_pc = 0
                final_points.append(points_inital)
                final_vtc_30.append(goal)
                final_vtc_135.append(goal_135)

        if state == "move to initial position":
            before_servo_pos = 0.20
            gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka2", "psm_main_insertion_joint"), before_servo_pos)
            main_ins_pos = gym.get_joint_position(envs[0], gym.get_joint_handle(envs[0], "kuka2", "psm_main_insertion_joint"))
            
            if np.allclose(main_ins_pos, 0.20, atol=0.001) :                           
                state = "set up velocity mode"
        ...
        if state == "get feature vector from VFH and PCA":
            ...
            point_cloud_publisher(ros_cloud)    # Publish to visualize with rviz
            ...
            final_vtc_30.append(feature_vector_30)
            final_vtc_135.append(feature_vector_135)
        ...
        if state == "get desired velocity and deform object":
            ...
            # else:
            # out = model(torch.from_numpy(goal - feature_vector_30).float())   # different between current and goal feature fector       
            ...
            gym.set_dof_target_velocity(envs[0], main_insertion_handle, 0.00)               
            
            # print("***Desired velocity: ", out[0][0].detach().numpy())            
            distance1 = np.linalg.norm(goal_135 - feature_vector_135)
            distance2 = np.linalg.norm(goal - feature_vector_30)
            print("***distance 135: ", distance1)     
            print("***distance 30: ", distance2)
            
            dist = np.asarray(pcd_goal.compute_point_cloud_distance(pcd))
            dist1 = np.amax(dist)
            dist2 = np.linalg.norm(dist)
            print("***distance max: ", dist1) 
            print("***distance norm: ", dist2)
            # pcd_goal = deepcopy(pcd)
            # print(feature_vector_135)
    ...
    with open('/home/baothach/shape_servo_data/uncertainty/uncertainty_vis_no_touching', 'wb') as handle:
