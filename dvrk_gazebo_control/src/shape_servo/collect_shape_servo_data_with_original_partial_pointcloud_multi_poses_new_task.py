...
if __name__ == "__main__":
    ...
    # cam_positions.append(gymapi.Vec3(0.05, -0.27, 0.3))
    ...
    data_recording_path = "/home/baothach/shape_servo_data/new_task/batch_1/data"
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "generate preshape":                   
            ...
            if cartesian_goal == None:
                ...
                cartesian_goal.position.z -= (ROBOT_Z_OFFSET + 0.01)
        ...
        if state == "move to preshape":            
            ...
            if np.allclose(dof_states['pos'][:8], pos_targets[:8], rtol=0, atol=0.1) and np.allclose(dof_states['pos'][8:], pos_targets[8:], rtol=0, atol=0.5)  :
        ...
        if state == "grasp object":             
            ...
            if dof_states['pos'][8] < 0.35:
                ...
                gym.set_joint_target_position(envs[i], gym.get_joint_handle(envs[i], "kuka2", "psm_tool_gripper1_joint"), 0.3)
                gym.set_joint_target_position(envs[i], gym.get_joint_handle(envs[i], "kuka2", "psm_tool_gripper2_joint"), -0.3)         
        ...
        if state == "get shape servo plan":
            ...
            delta_z = 0#np.random.uniform(low = 0, high = max_z) 
            ...
            #     # delta_y = np.random.choice([0.00, 0.02])  
            #     # delta_z = np.random.choice([0, 0.05])  
            #     delta_x = 0.05
            #     delta_y = 0.05 
            #     delta_z = 0.01  
            ...
            cartesian_pose.position.z = max(0.009 - ROBOT_Z_OFFSET, current_pose["pose"]["p"]["z"] - ROBOT_Z_OFFSET + delta_z)
        ...
        if state == "move to goal":
            ...
            if not(20 in contacts or 21 in contacts):  # lose contact w 1 robot
                ...
                if traj_index == len(plan_traj):
                    ...
                    for j, cur_pose in enumerate(poses_on_trajectory):
                        ...
                        pcs = (pc_on_trajectory[j], pc_goal)
                        ...
                        data = {"point clouds": pcs, "positions": [delta_x, delta_y, delta_z], "grasp_pose": cur_pose["pose"]}
        ...
        if state == "reset":   
            ...
            if np.allclose(dof_states_2, pos_targets, rtol=0, atol=0.5):
