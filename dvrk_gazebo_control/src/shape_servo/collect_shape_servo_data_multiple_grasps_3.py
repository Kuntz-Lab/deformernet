...
if __name__ == "__main__":
    ...
    # cam_positions.append(gymapi.Vec3(0.1, -0.5, 0.15))
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "move to goal":
            ...
            if (not(20 in contacts or 21 in contacts) or not(9 in contacts and 10 in contacts)):  # lose contact w robot 2 or robot 1
                ...
                image_path = "/home/baothach/shape_servo_data/keypoints/batch3/color/image_" + str(image_pair_count) + ".png"
                gym.write_camera_image_to_file(sim, envs_obj[0], cam_handles[0], gymapi.IMAGE_COLOR, image_path)                  
                image_path = "/home/baothach/shape_servo_data/keypoints/batch3/depth/image_" + str(image_pair_count) + ".png"
                ...
                if frame_count % 10 == 0:
                    ...
                    gym.write_camera_image_to_file(sim, envs_obj[0], cam_handles[0], gymapi.IMAGE_DEPTH, image_path)
                    image_pair_count += 1
                    # image = gym.get_camera_image(sim, envs_obj[0], cam_handles[0], gymapi.IMAGE_DEPTH)
                    # print(max(np.array(image).flatten()))
                    # print(min(np.array(image).flatten()))
                frame_count += 1           
