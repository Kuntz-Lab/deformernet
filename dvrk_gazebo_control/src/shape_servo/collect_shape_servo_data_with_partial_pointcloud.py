...
if __name__ == "__main__":
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            max_x = 0.04 + (current_pose["pose"]["p"]["y"]+0.43)*(0.07-0.04)/(-0.375+0.43)
            max_y = 0.02 + (current_pose["pose"]["p"]["y"]+0.43)*(0.06-0.02)/(-0.375+0.43)              
            max_z = 0.05 + (current_pose["pose"]["p"]["y"]+0.43)*(0.07-0.05)/(-0.375+0.43) 
        ...
        if state == "move to goal":
            ...
            if (not(20 in contacts or 21 in contacts) or not(9 in contacts and 10 in contacts)):  # lose contact w robot 2 or robot 1
                ...
                if traj_index == len(plan_traj):
                    ...
                    #     data = {"point clouds": sparse_pcs, "positions": [delta_x, delta_y, delta_z], "grasp_pose": current_pose["pose"]}
