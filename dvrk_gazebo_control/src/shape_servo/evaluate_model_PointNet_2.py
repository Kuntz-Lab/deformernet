...
if __name__ == "__main__":
    ...
    goal_idx = 0
    pc_goal = torch.from_numpy(np.swapaxes(data["point clouds"][goal_idx],0,1)).float()
    # print("***Ground Truth: ", data["positions"][30])
    # print(pc_goal.shape)
    ...
    model.load_state_dict(torch.load("/home/baothach/shape_servo_data/weights/PointNet/training_end_to_end_new_task"))    
    ...
    while (not gym.query_viewer_has_closed(viewer)) and (not all_done):
        ...
        if state == "get desired velocity and deform object":
            ...
            desired_position = out[0][0].detach().numpy()
            gym.set_dof_target_position(envs[0], main_insertion_handle, 0.213 + desired_position)             
            
            print("***Desired position this time: ", out[0][0].detach().numpy())  
            print("***Ground Truth: ", data["positions"][goal_idx])
