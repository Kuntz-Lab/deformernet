...
from pointconv_with_kp import PointNetShapeServoKp2
import torch
...
if __name__ == "__main__":
    ...
    model = PointNetShapeServoKp2(normal_channel=False)
    model.load_state_dict(torch.load("/home/baothach/shape_servo_data/keypoints/combined_w_shape_servo/batch1/weights/run3/batch_1-epoch 112"))  
    model.eval()

    args_kp = config(use_gym = True, config = "/home/baothach/keypoints/configs/keypoints_celeba.yaml/")
    # args.config = "/home/baothach/keypoints/configs/keypoints_celeba.yaml/"

    args_kp.load = "/home/baothach/keypoints/data/models/keypoints/F/run_7/best/"
    kp_network = keynet.make(args_kp).to(device)
    kp_network.eval()

#    Get goal pc:
    with open('/home/baothach/shape_servo_data/keypoints/combined_w_shape_servo/batch1/goal_data/sample 0.pickle', 'rb') as handle:
        data = pickle.load(handle)
        pc_goal = data["point clouds"][1].toarray().reshape(256, 256, 3)     
        pcd_goal = open3d.geometry.PointCloud()
        pcd_goal.points = open3d.utility.Vector3dVector(pc_goal.reshape((-1, 3))) 
        goal_position = data["positions"]   
    
    
    with torch.no_grad():
        pc_goal_tensor = torch.tensor(pc_goal).permute(2,0,1).unsqueeze(0).float().to(device) 
        _, _, k_goal, _, _, _ = kp_network(pc_goal_tensor, pc_goal_tensor)
        k_goal = k_goal.squeeze().detach().cpu().numpy()
        keypoints_goal = torch.tensor(convert_pc_to_kp_cloud(k_goal, pc_goal)).permute(1, 0).float() 
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "generate preshape":       
            ...
            # point_cloud_change = np.array(current_pc)-np.array(goal_pc_numpy)
            # point_cloud_change = [np.linalg.norm(point) for point in  point_cloud_change]
            # initial_pc = deepcopy(current_pc)
            # current_pc = torch.from_numpy(np.swapaxes(current_pc,0,1)).float()            
            # desired_mani_pose = model_2(current_pc.unsqueeze(0), goal_pc.unsqueeze(0))[0].detach().numpy()*(0.001)
            # print("manipulation pose: ", desired_mani_pose)
        ...
        if state == "get shape servo plan":
            ...
            pcd = open3d.geometry.PointCloud()
            pcd.points = open3d.utility.Vector3dVector(current_pc.reshape((-1,3)))  
            ...
            with torch.no_grad():
                pc_tensor = torch.tensor(current_pc).permute(2,0,1).unsqueeze(0).float().to(device) 
                _, _, k, _, _, _ = kp_network(pc_tensor, pc_tensor)
                k = k.squeeze().detach().cpu().numpy()
                keypoints_current = torch.tensor(convert_pc_to_kp_cloud(k, current_pc)).permute(1, 0).float()            
            ...
            desired_position = model(keypoints_current.unsqueeze(0), keypoints_goal.unsqueeze(0))[0].detach().numpy()*(0.001)  
        ...
        if state == "move to goal":           
            ...
            if traj_index == 3 or traj_index == len(plan_traj):
                traj_index = 0  
