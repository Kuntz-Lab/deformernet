...
if __name__ == "__main__":
    ...
    soft_pose.p = gymapi.Vec3(0.0, 0.44-two_robot_offset, 0.02)
    ...
    # Set up PCA:
    with open("/home/baothach/shape_servo_data/feature_vectors_135", 'rb') as f:
    ...
    # Get goal pc:
    with open('/home/baothach/shape_servo_data/goal_1c', 'rb') as handle:
    ...
    ros_cloud = orh.o3dpc_to_rospc(pcd_goal, frame_id = "map")
    goal_135 = np.array(VFH_client(ros_cloud)).reshape(1, -1)  # np array size (1,135)
    goal_30 = pca.transform(goal_135) # np array size (1,30)         
    ...
    ## VFH:
    model = Net()
    model.load_state_dict(torch.load("/home/baothach/shape_servo_data/weights/VFH_PCA_with_relu"))      
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get plan":
            ...
            model.eval()
            desired_position = model(torch.from_numpy(goal_30 - feature_vector_30).float())[0].detach().numpy()*(0.001)  
    ...
    with open('/home/baothach/shape_servo_data/visualization/chamfer_VFH_with_relu', 'wb') as handle:
