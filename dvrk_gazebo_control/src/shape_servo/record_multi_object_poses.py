...
def move_object(saved_object_state):
    point_cloud = saved_object_state.numpy()[:, :3]   
    pcd = open3d.geometry.PointCloud()
    pcd.points = open3d.utility.Vector3dVector(point_cloud)
    pcd2 = deepcopy(pcd.translate((0.1, 0, 0)))
    return torch.from_numpy(np.asarray(pcd2.points)).float()
...
if __name__ == "__main__":
    ...
    for i in range(num_envs):
        ...
        for k in range(200):
            ...
            delta_y = np.random.uniform(low = -0.03, high = 0.03)
            ...
            soft_actor = gym.create_actor(env_obj, soft_asset, object_pose, "soft", k, 0)
            object_handles.append(soft_actor)
    ...
    # dc_client = GraspDataCollectionClient()
    data_recording_path = "/home/baothach/shape_servo_data/keypoints/combined_w_shape_servo/batch1/goal_data"
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            if frame_count >= 6:
                ...
                print(saved_object_state.shape)
            # print((saved_object_state[:,:3] == torch.zeros((3486,3))).all())


                data = saved_object_state  # multi poses
                with open('/home/baothach/shape_servo_data/keypoints/combined_w_shape_servo/record_multi_object_poses/batch2(200).pickle', 'wb') as handle:
