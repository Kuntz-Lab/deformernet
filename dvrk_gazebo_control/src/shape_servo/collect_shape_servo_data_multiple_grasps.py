...
if __name__ == "__main__":
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            # delta_x = np.random.uniform(low = -0.06, high = 0.06)   
            # delta_y = np.random.uniform(low = 0.0, high = 0.05)
            # delta_z = np.random.uniform(low = 0.0, high = 0.06) 
        ...
        if state == "move to goal":
            ...
            if (not(20 in contacts or 21 in contacts) or not(9 in contacts and 10 in contacts)):
    ...
    # data = {"point clouds": final_point_clouds, "positions": final_desired_positions, "saved intial state":saved_object_state}
    # with open('/home/baothach/shape_servo_data/multi_grasps/goal_1', 'wb') as handle:
    #     pickle.dump(data, handle, protocol=pickle.HIGHEST_PROTOCOL)
