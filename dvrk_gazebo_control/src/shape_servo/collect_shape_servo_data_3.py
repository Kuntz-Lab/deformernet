...
if __name__ == "__main__":
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get plan":
            ...
            # delta_y = 0.03 
            # delta_z = 0.025  
            # delta_x = np.random.choice([-0.04, 0.04])   
            # delta_y = np.random.choice([0.0, 0.03]) 
            # delta_z = np.random.choice([0.0, 0.025])              
            delta_x = np.random.uniform(low = -0.02, high = 0.02)   
            delta_y = np.random.uniform(low = 0.0, high = 0.015)
            delta_z = np.random.uniform(low = 0.0, high = 0.012) 
        ...
        if sample_count % 10 == 0:
            ...
            with open('/home/baothach/shape_servo_data/batch_3(same_mani_points)', 'wb') as handle:
