    ...
    for k in range(cam_width):
        for t in range(cam_height):
            ...
            if p2[0, 2] > 0.005:
                ...
                depth_buffer[t, k] = max_depth
    return points, convert_depth_map_to_png(-depth_buffer)        

def convert_depth_map_to_png(depth_map):
    flatten_depth_map = depth_map.flatten()
    flatten_depth_map = [x for x in list(flatten_depth_map) if x != 3]
    max_depth = max(flatten_depth_map)
    # print(max_depth)

    min_depth = min(flatten_depth_map)

    depth_map = np.where(depth_map == 3, min_depth, depth_map)
...
if __name__ == "__main__":
    ...
    for i in range(num_envs):
        ...
        delta_x = -0.03 #np.random.uniform(low = -0.03, high = 0.03)
    ...
    # reset_soft_obj = True
    # random_stuff = True
    image_recording_path = "/home/baothach/shape_servo_data/keypoints/batch6/depth"
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            #     open3d.visualization.draw_geometries([pcd])   
            #     img = Image.fromarray(np.uint8(depth_image), mode = 'L')
            #     img.show()
            #     # depth_buffer = gym.get_camera_image(sim, envs_obj[i], cam_handles[i], gymapi.IMAGE_DEPTH)
            #     # img = Image.fromarray(np.uint8(depth_buffer), mode = 'L')
            #     # img.show()
            #     # image_path = "/home/baothach/Downloads/random" + ".png"
            #     # gym.write_camera_image_to_file(sim, envs_obj[0], cam_handles[0], gymapi.IMAGE_DEPTH, image_path)  
