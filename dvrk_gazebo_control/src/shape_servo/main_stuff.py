...
if __name__ == "__main__":
    ...
    state = "home"
    # goal = FPFH_135_vectors[20].reshape(1, -1)  # desired feature vector for shape servoing
    ...
    with open("/home/baothach/shape_servo_data/goal_feature_vector_135.txt", 'rb') as f:
        goal_135 = np.array(pickle.load(f))  
    ...
    while (not gym.query_viewer_has_closed(viewer)) and (not all_done):
                ...
                for j in range(cam_height):
                    ...
                    if seg_buffer[j, k] == 0:
                        ...
                        if p2[0, 2] > 0.005:
                            ...
                            color.append(0)
            state = "get data and train model"

        if state == "get data and train model":
            rospy.loginfo("**Current state: " + state)
            # Get feature vector
            ...
            if first_time_step:     # First time step does not have previos feature vector --> = 0
                input = torch.from_numpy(feature_vector_30 - feature_vector_30).float()
                first_time_step = False
            else:
                input = torch.from_numpy(feature_vector_30 - old_feature_vector_30).float()
            main_ins_vel = gym.get_joint_velocity(envs[0], gym.get_joint_handle(envs[0], "kuka2", "psm_main_insertion_joint"))  # current main insertion joint velocity
            target = torch.tensor([[main_ins_vel]]).float()
            train(model, device, optimizer, epoch, input, target)
            epoch += 1
        ...
        if state == "get desired velocity and deform object":
            ...
            if np.allclose(main_ins_pos, 0.25, atol=0.002):
                gym.set_dof_target_velocity(envs[0], main_insertion_handle, -0.01)
                # print("***Too close to 0.24. Desired velocity: -0.05")
            elif np.allclose(main_ins_pos, 0.225, atol=0.005) or main_ins_pos < 0.220:
                gym.set_dof_target_velocity(envs[0], main_insertion_handle, 0.05) 
                # print("***Too close to 0.225. Desired velocity: 0.05")          
            else:
                out = model(torch.from_numpy(goal - feature_vector_30).float().to(device))   # desired velocity -- difference between current and goal feature fector       
                desired_velocity = np.clip(out[0][0].cpu().detach().numpy(), -0.01, 0.01)
                gym.set_dof_target_velocity(envs[0], main_insertion_handle, desired_velocity)                  
                gym.set_dof_target_velocity(envs[0], main_insertion_handle, desired_velocity)             
                # print("***Desired velocity: ", desired_velocity)    
                # print("***Actual command velocity: ", out[0][0].cpu().detach().numpy())   

            distance = np.linalg.norm(goal - feature_vector_30)
            print("***distance: ", distance)     

            old_feature_vector_30 = feature_vector_30   # save feature vector for next time step   
    ...
    torch.save(model.state_dict(), "/home/baothach/shape_servo_data/weights/shape_servo_weights")
