...
if __name__ == "__main__":
    ...
    pose.p = gymapi.Vec3(offset_pc, -two_robot_offset, ROBOT_Z_OFFSET)
    #pose.r = gymapi.Quat(-0.707107, 0.0, 0.0, 0.707107)


    pose_2 = gymapi.Transform()
    pose_2.p = gymapi.Vec3(offset_pc, 0.0, ROBOT_Z_OFFSET)
    ...
    soft_pose.p = gymapi.Vec3(offset_pc, 0.44-two_robot_offset, 0.02)
    ...
    dof_props["stiffness"][:8].fill(500.0)
    dof_props["damping"][:8].fill(100.0)
    ...
    # Get goal pc:
    with open('/home/baothach/shape_servo_data/goal_1', 'rb') as handle:
        data = pickle.load(handle)
    goal_pc_numpy = data["point clouds"][0][1]
    goal_pc = torch.from_numpy(np.swapaxes(goal_pc_numpy,0,1)).float() 
    goal_position = data["positions"]
    pcd_goal = open3d.geometry.PointCloud()
    pcd_goal.points = open3d.utility.Vector3dVector(np.array(goal_pc_numpy))      
    pcd_goal = pcd_goal.translate((offset_pc, 0, 0))
    
    # pcd_goal_2 = deepcopy(pcd_goal).translate((-2, 0, 0))      
    # mesh = open3d.geometry.TriangleMesh.create_coordinate_frame()
    # pcd_goal_2.paint_uniform_color([1, 0.706, 0])
    # open3d.visualization.draw_geometries([pcd_goal, pcd_goal_2, mesh])    
    
    # Set up DNN:
    model = PointNetShapeServo(normal_channel=False)
    model.load_state_dict(torch.load("/home/baothach/shape_servo_data/weights/PointNet/batch_2"))  
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get plan":
            ...
            print("ground truth: ", goal_position[0])
            ...
            cartesian_pose.position.x = -current_pose["pose"]["p"]["x"] + offset_pc + delta_x
        ...
        if state == "move to goal":           
            ...
            #     delta_x = -(final_pose["pose"]["p"]["x"] - current_pose["pose"]["p"]["x"])
            #     delta_y = -(final_pose["pose"]["p"]["y"] - current_pose["pose"]["p"]["y"])
            #     delta_z = final_pose["pose"]["p"]["z"] - current_pose["pose"]["p"]["z"]
    ...
    # data = chamfer_distances
    # with open('/home/baothach/shape_servo_data/visualization/chamfer_DeformNet', 'wb') as handle:
