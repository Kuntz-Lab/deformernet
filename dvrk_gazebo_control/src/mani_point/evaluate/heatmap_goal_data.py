...
if __name__ == "__main__":
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            delta_x, delta_y, delta_z = 0.03, max_y * 2/5, max_z * 5/5 
        ...
        if state == "move to goal":
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if frame_count == 0:
                    # pc_init = get_partial_point_cloud(i)
                    # full_pc_init = get_point_cloud()
                    init_pose = deepcopy(gym.get_actor_rigid_body_states(envs[i], kuka_handles_2[i], gymapi.STATE_POS)[-3])                        
                ...
                if traj_index == len(plan_traj):
                    ...
                    data = {"full pcs": full_goal_pcs, "partial pcs": partial_goal_pcs, "mani_point": init_pose["pose"], "obj_name": args.obj_name, \
                    ...
                    with open(os.path.join(data_recording_path, "goal " + str(data_point_count) + ".pickle"), 'wb') as handle:
