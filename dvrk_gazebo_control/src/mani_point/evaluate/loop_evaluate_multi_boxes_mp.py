...
def run_evaluate_loop(headless, prim_name, obj_type, inside, num_obj=10):
    for i in range(0, num_obj):
        ...
        os.system(f"rosrun dvrk_gazebo_control evaluate_{prim_name}_predicted_mp.py --flex --headless {str(headless)} --obj_name {prim_name}_{i}\
                    --obj_type {obj_type} --inside {str(inside)} --no_rot True --mp_method dense_predictor")

def run_collect_goals_loop(headless, prim_name, obj_type, inside, num_obj=10):
    for i in range(0, num_obj):
        os.system("source devel/setup.bash")

        os.system(f"rosrun dvrk_gazebo_control collect_goals_{prim_name}_mp.py --flex --headless {str(headless)} --obj_name {prim_name}_{i}\
