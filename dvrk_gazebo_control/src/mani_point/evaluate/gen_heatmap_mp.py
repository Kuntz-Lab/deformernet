...
if __name__ == "__main__":
    ...
    args = gymutil.parse_arguments(
        ...
        custom_parameters=[
            ...
            {"name": "--goal_idx", "type": int, "default": 1, "help": "index of the recorded goal pc"},
    ...
    # sample_count = 0
    heatmap_path = "/home/baothach/shape_servo_data/manipulation_points/multi_boxes_1000Pa/heatmaps"   
    with open(os.path.join(heatmap_path, "goal_data", f"goal {args.goal_idx}.pickle"), 'rb') as handle:
    ...
    result_path = os.path.join(heatmap_path, "results")
    # log_dir =  "/home/baothach/shape_servo_data/manipulation_points/box/logs"
    first_time = True
    frame_count = 0
    sample_count = args.goal_idx
    mp_count = 0
    max_sample_count = sample_count + 1
    max_mp_count = 101 # 1 for ground truth and 100 random MPs
    recording_offset = 0
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            if frame_count == 5:
                ...
                if mp_count == 0:
                    ...
                    init_pc_numpy = down_sampling(get_partial_point_cloud(i))
                    # init_pc_tensor = torch.from_numpy(init_pc_numpy).unsqueeze(0).float()    
       

                    # print("initial chamfer:", np.sqrt(chamferDist(goal_pc_tensor.permute(0,2,1), init_pc_tensor).item()))
        ...
        if state == "get shape servo plan":
            ...
            if chamfer_dist >= min_chamfer_dist:
                current_pc_tensor = torch.from_numpy(current_pc).permute(1,0).unsqueeze(0).float()
                with torch.no_grad():
                    desired_position = model(current_pc_tensor, goal_pc_tensor)[0].detach().numpy()*(0.001) 
        ...
        if state == "reset":   
            ...
            if len(recorded_mps) != 0:  
                ...
                final_data = {"mani_point": mani_point, "chamfer": final_chamfer, \
                               "gt mani_point": gt_mp, "obj_name": args.obj_name}                                                                   


            with open(os.path.join(result_path, f"sample {sample_count} mp {idx}.pickle"), 'wb') as handle:
