...
from VI_PointConv_utils import get_mp_VI_PointConv
...
def get_mp_keypoint(partial_init_pc, partial_goal_pc, vis=False):
    ...
    with torch.no_grad():
        ...
        heatmap, mani_point, mani_point_xyz = get_kp_pc_heatmap(k1, k2, source_pc, target_pc, get_mani_point = True, thickness = 1, N=10, rm_outliers=False)
        ...
        if vis:
            pcd = open3d.geometry.PointCloud()
            pcd.points = open3d.utility.Vector3dVector(down_sampling(partial_init_pc.reshape(-1,3)))
            pcd.paint_uniform_color([0,0,0])
            open3d.visualization.draw_geometries([pcd, mani_point, gt_mani_point])   
...
if __name__ == "__main__":
    ...
    goal_count = 0#0 #int(args.obj_name[4:])
    ...
    mp_seg_model = ManiPointSegment(num_classes=2).to(device)
    # from architecture_seg_2 import ManiPointSegment2
    # mp_seg_model = ManiPointSegment().to(device)
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "generate preshape":                   
            ...
            with torch.no_grad():
                ...
                ### VI PointConv:
                elif args.mp_method == "VI_PointConv":
                    best_mp = get_mp_VI_PointConv(init_pc_numpy, goal_pc_numpy, gt_mp=gt_mp, vis=True)
                # # Heuristic
