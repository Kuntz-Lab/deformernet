...
if __name__ == "__main__":
    ...
    rot_angle = np.pi/2 - np.pi/8#np.pi/2 + np.random.uniform(low = -np.pi/4.5, high = np.pi/4.5) 
    ...
    if args.inside:
        ...
        goal_recording_path = os.path.join(main_path, "goal_data", args.obj_type, "test_lung_tung")
    else:
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            delta_x, delta_y, delta_z = 0.08, 0.04, 0.12     
        ...
        if state == "move to goal":
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if traj_index == len(plan_traj) or terminate_count >= 10:
                    traj_index = 0  
                    ...
                    goal_data = {"full pcs": full_goal_pcs, "partial pcs": partial_goal_pcs, "positions": [delta_x, delta_y, delta_z], "mani_point": init_pose["pose"],
