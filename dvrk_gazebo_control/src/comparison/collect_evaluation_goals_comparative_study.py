...
if __name__ == "__main__":
    ...
    rospy.logerr(f"thickness: {thickness}")
    ...
    goal_recording_path = "/home/baothach/shape_servo_data/comparative_study/goal_data" 
    ...
    data_point_count = len(os.listdir(goal_recording_path))
    max_data_point_count = 10
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            delta_x = random.uniform(*random.choice([(-max_x, -max_x/3), (max_x/3, max_x)]))
        ...
        if state == "move to goal":
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if action is not None and gym.get_sim_time(sim) - closed_loop_start_time <= 5: 
                    ...
                    partial_goal_pc = get_partial_point_cloud(i)
                    full_goal_pc = get_point_cloud()                   

                    pcd = open3d.geometry.PointCloud()
                    pcd.points = open3d.utility.Vector3dVector(partial_goal_pc)     
                    open3d.visualization.draw_geometries([pcd])                 

                    goal_data = {"full pc": full_goal_pc, "partial pc": partial_goal_pc, \
        ...
        if  goal_point_count >= max_goal_count:                     
            ...
            with open(os.path.join(goal_recording_path, f"sample {data_point_count}.pickle"), 'wb') as handle:
                pickle.dump(final_data, handle, protocol=pickle.HIGHEST_PROTOCOL) 

            print(f"Done data point {data_point_count}")
            data_point_count += 1
            goal_point_count = 0
