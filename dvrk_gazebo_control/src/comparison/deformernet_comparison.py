...
#    Get goal pc:
    ...
    with open(os.path.join(data_recording_path, "sample " + str(7) + ".pickle"), 'rb') as handle:
        data = pickle.load(handle)
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            start_deformernet_time = timeit.default_timer()
            with torch.no_grad():
                desired_position = model(current_pc.unsqueeze(0), goal_pc.unsqueeze(0))[0].detach().numpy()*(0.001)  
            total_computation_time += (timeit.default_timer()-start_deformernet_time)
        ...
        if state == "move to goal":           
            ...
            if action is None or timeit.default_timer() - closed_loop_start_time >= 10:   
                ...
                total_computation_time += tvc_behavior.computation_time  
