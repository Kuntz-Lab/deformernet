...
if __name__ == "__main__":
    ...
    chamfer_recording_path = "/home/baothach/shape_servo_data/comparative_study/deformernet/results"
    os.makedirs(chamfer_recording_path, exist_ok=True)
    
    with open(os.path.join(goal_recording_path, f"sample {goal_count}.pickle"), 'rb') as handle:
        goal_datas = pickle.load(handle) 
    goal_pc_numpy = down_sampling(goal_datas["partial pc"])   # first goal pc
    ...
    full_pc_goal = goal_datas["full pc"]
    ...
    saved_obj_contact_state = goal_datas["obj contact state"]
    saved_robot_contact_state = goal_datas["robot contact state"]
    print("=========vel_limits",vel_limits)
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "reset":   
            ...
            if goal_count < max_goal_count:
                with open(os.path.join(goal_recording_path, f"sample {goal_count}.pickle"), 'rb') as handle:
                    goal_datas = pickle.load(handle) 
                goal_pc_numpy = down_sampling(goal_datas["partial pc"])
                goal_pc_tensor = torch.from_numpy(goal_pc_numpy).permute(1,0).unsqueeze(0).float().to(device)
                pcd_goal = open3d.geometry.PointCloud()
                pcd_goal.points = open3d.utility.Vector3dVector(goal_pc_numpy) 
                pcd_goal.paint_uniform_color([1,0,0])
        ...
        if  goal_count >= max_goal_count:                    
            ...
            with open(os.path.join(chamfer_recording_path, "deformernet_result.pickle"), 'wb') as handle:
