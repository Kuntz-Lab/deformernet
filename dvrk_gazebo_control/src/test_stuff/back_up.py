...
if __name__ == "__main__":
    ...
    rigid_pose.p = gymapi.Vec3(0, -0.4+two_robot_offset, 0.03)
    ...
    soft_pose.p = gymapi.Vec3(0.065, -0.40+two_robot_offset, 0.1)
    soft_pose.r = gymapi.Quat(0.0, 0.0, 0.707107, 0.707107)   #for banana
    # soft_pose.r = gymapi.Quat(0.7071068, 0, 0, 0.7071068)
    soft_thickness = 0.0001    # important to add some thickness to the soft body to avoid interpenetrations
    ...
    for i in range(num_envs):
        ...
        # add rigid obj
        rigid_actor = gym.create_actor(env, rigid_asset, rigid_pose, 'rigid', i, 0)
...
#    Get goal pc:
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "generate preshape":                   
            ...
            # target_pose = [-cartesian_goal.position.x, -cartesian_goal.position.y, cartesian_goal.position.z-ROBOT_Z_OFFSET-0.01,
            #                 cartesian_goal.orientation.x, cartesian_goal.orientation.y,cartesian_goal.orientation.z,cartesian_goal.orientation.w]
