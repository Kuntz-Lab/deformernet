...
if __name__ == "__main__":
    ...
    # soft_asset_file = "test.urdf"
    soft_asset_file = "cylinder.urdf"
    ...
    # soft_pose.r = gymapi.Quat(0.0, -1, 0, 0)
    ...
    soft_asset_file = "round_rod.urdf"
    soft_pose_2 = deepcopy(soft_pose)
    soft_pose_2.p = gymapi.Vec3(0.1, 0.50-two_robot_offset, 0.1)   
    soft_asset_2 = gym.load_asset(sim, asset_root, soft_asset_file, asset_options)
    
    soft_asset_file = "square_rod.urdf"
    soft_pose_3 = deepcopy(soft_pose)
    soft_pose_3.p = gymapi.Vec3(0.2, 0.50-two_robot_offset, 0.1)
    soft_pose_3.r = gymapi.Quat(0.0, 0.0, 0.707107, 0.707107)      
    soft_asset_3 = gym.load_asset(sim, asset_root, soft_asset_file, asset_options)        
    
    soft_asset_file = "hemis.urdf"
    soft_pose_4 = gymapi.Transform()
    soft_pose_4.p = gymapi.Vec3(0.0, 0.3-two_robot_offset, 0.1)     
    soft_asset_4 = gym.load_asset(sim, asset_root, soft_asset_file, asset_options) 


    asset_root = "/home/baothach/sim_data/BigBird/BigBird_urdf_new"
    soft_asset_file = "cheez_it_white_cheddar.urdf"
    soft_pose_5 = deepcopy(soft_pose)
    soft_pose_5.p = gymapi.Vec3(0.2, 0.3-two_robot_offset, 0.1)    
    soft_asset_5 = gym.load_asset(sim, asset_root, soft_asset_file, asset_options)  
    ...
    for i in range(num_envs):
        ...
        soft_actor_2 = gym.create_actor(env_obj, soft_asset_2, soft_pose_2, "soft", i, 0)
        soft_actor_3 = gym.create_actor(env_obj, soft_asset_3, soft_pose_3, "soft", i, 0)
        soft_actor_4 = gym.create_actor(env_obj, soft_asset_4, soft_pose_4, "soft", i, 0)
        soft_actor_5 = gym.create_actor(env_obj, soft_asset_5, soft_pose_5, "soft", i, 0)
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if start_vis_cam: 
            if vis_frame_count % 40 == 0:
                ...
                # im[goal_xs, goal_ys, :] = [255,0,0,255]
        ...
        if state == "generate preshape":                   
            ...
            if cartesian_goal == None:
                ...
                # cartesian_goal.position.x = 0.064#0.023#0.064#-0.072#-0.081#-0.1134
                # cartesian_goal.position.y = -0.248#-0.252#-0.238#-0.341#-0.341#-0.33
                # cartesian_goal.position.z = 0.027#0.032#0.027#0.023#0.024#0.0295  
