...
def get_point_cloud(vis=False):
    ...
    if vis:
        pcd = open3d.geometry.PointCloud()
        pcd.points = open3d.utility.Vector3dVector(np.array(down_sampling(point_cloud, num_point=1024)))
        pcd.paint_uniform_color([1,0,0])
        
        tet_mesh = open3d.geometry.TetraMesh()
        tet_mesh = tet_mesh.create_from_point_cloud(pcd)[0]
        # tet = tet_mesh.paint_uniform_color(np.array([[1],[0],[0]]))
        open3d.visualization.draw_geometries([tet_mesh]) 
...
if __name__ == "__main__":
    ...
    soft_pose.p = gymapi.Vec3(0.035, 0.40-two_robot_offset, 0.01)  # 0.08
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            if frame_count == 10:
                ...
                # print("=====partial pc: ", get_partial_point_cloud(i).shape)
                ...
                current_pc = get_point_cloud(vis=False)
                pcd = open3d.geometry.PointCloud()
        ...
        if state == "get shape servo plan":
            ...
            # INTERPOLATION:
            bl = [0.08658331, -0.39884162, 0.09282317]  # bottom left
            br = [-0.013941547, -0.40259874, 0.105266504]
            tl = [0.08379825, -0.46419957, 0.09617778]    # top left
            tr = [-0.02220822, -0.46465507, 0.10152289]  
            max_x = -0.11 + (current_pose["pose"]["p"]["x"]-br[0])*(0.12+0.13)/(bl[0]-br[0])
            max_y = 0.07 + (current_pose["pose"]["p"]["y"]-tl[1])*(0.13-0.07)/(bl[1]-tl[1])              
            max_z = 0.1
            ...
            # print("soft_contacts", [contact[4] for contact in gym.get_soft_contacts(sim)])
            if max_x > 0:
                delta_x = np.random.uniform(low = 0, high = max_x)   
            else:
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if traj_index == len(plan_traj):
                    ...
                    for j, cur_pose in enumerate(poses_on_trajectory):
                        ...
                        # data = {"full pcs": full_pcs, "partial pcs": partial_pcs, "positions": [delta_x, delta_y, delta_z], "grasp_pose": cur_pose["pose"]}
        ...
        if state == "reset":   
            ...
            # dof_states_2 = robot.get_full_joint_positions()
            
            # if np.allclose(dof_states_2[:8], pos_targets[:8], rtol=0, atol=0.01):# and np.allclose(dof_states_2[8:], pos_targets[8:], rtol=0, atol=0.5):
            # if np.allclose(dof_states_2, pos_targets, rtol=0, atol=0.1):
