...
init_ee_joints = [0.8, 0.1]
def init():
    for i in range(num_envs):
        # # Kuka 2
        davinci_dof_states = gym.get_actor_dof_states(envs[i], kuka_handles_2[i], gymapi.STATE_NONE)
        davinci_dof_states['pos'][8] = init_ee_joints[0]  #1.5
        davinci_dof_states['pos'][9] = init_ee_joints[1]      #0.8
...
if __name__ == "__main__":
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            gym.set_joint_target_position(envs[i], gym.get_joint_handle(envs[i], "kuka2", "psm_tool_gripper1_joint"), init_ee_joints[0])
            gym.set_joint_target_position(envs[i], gym.get_joint_handle(envs[i], "kuka2", "psm_tool_gripper2_joint"), init_ee_joints[1])  
            if frame_count == 10:
        ...
        if state == "generate preshape":                   
            ...
            euler = [np.pi, np.pi/4, np.pi/2]
            # print("+++++++++++++++:", transformations.euler_from_quaternion([0, 0.707107, 0.707107, 0]))
            quat = list(transformations.quaternion_from_euler(*euler))
        ...
        if state == "get shape servo plan":
            ...
            state = "move to goal"
            # rospy.logerr(f"Plan count: {plan_count}")
            # plan_count += 1
