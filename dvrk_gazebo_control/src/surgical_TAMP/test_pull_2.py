...
if __name__ == "__main__":
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka2", "psm_main_insertion_joint"), 0.15)            
            if frame_count == 10:
        ...
        if state == "grasp object":             
            rospy.loginfo("**Current state: " + state)
            gym.set_joint_target_position(envs[i], gym.get_joint_handle(envs[i], "kuka2", "psm_tool_gripper1_joint"), 0.8)
            gym.set_joint_target_position(envs[i], gym.get_joint_handle(envs[i], "kuka2", "psm_tool_gripper2_joint"), 0.1)         
        ...
        if state == "get shape servo plan":
            ...
            delta_alpha, delta_beta, delta_gamma = np.pi/2, np.pi/2, 3*np.pi/4 
