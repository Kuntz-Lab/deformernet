    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            if frame_count == 10:
                ...
                if visualization:
                    pcd = pcd_ize(get_partial_point_cloud(obj_name="tissue"), color=[0,0,0])
                    pcd_plane = pcd_ize(visualize_plane(constrain_plane, x_range=0.15, y_range=[-0.57,-0.27], z_range=[0.0,0.18], num_pts=2000), color=[1,0,0])
                    pcd_kidney = pcd_ize(kidney_pc, color=[0,1,0])
