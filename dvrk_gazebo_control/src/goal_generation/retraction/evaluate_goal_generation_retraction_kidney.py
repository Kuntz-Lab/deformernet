...
if __name__ == "__main__":
    ...
    model = DeformerNet(use_mp_input=True).to(device)    
    weight_path = f"/home/baothach/shape_servo_data/rotation_extension/multi_box_1kPa/weights/run1_w_rot_w_MP/"        
    deformernet_epoch_num = 524
    model.load_state_dict(torch.load(os.path.join(weight_path, f"epoch {deformernet_epoch_num}")))  
    model.eval()

    sys.path.append(f"/home/baothach/goal_generation_net/")
    from model import GoalGenRetractKidney
    goal_model = GoalGenRetractKidney(num_points=512, embedding_size=256).to(device)
    goal_model.load_state_dict(torch.load("/home/baothach/shape_servo_data/goal_generation/retraction_kidney/weights/run1/epoch 150"))
    goal_model.eval()
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            if frame_count == 10:
                ...
                if first_time:
                    ...
                    pc_tensor = torch.from_numpy(get_partial_point_cloud(obj_name="tissue")).permute(1,0).unsqueeze(0).float().to(device)
                    kidney_pc_tensor = torch.from_numpy(kidney_pc).permute(1,0).unsqueeze(0).float().to(device)
                    goal_pc_tensor = goal_model(pc_tensor, kidney_pc_tensor)
                    
                    pcd_goal = pcd_ize(goal_pc_tensor.squeeze().cpu().detach().numpy().transpose(1,0))
                ...
                if visualization:
                    ...
                    pcd_plane = pcd_ize(visualize_plane(constrain_plane, x_range=0.15, y_range=[-0.57,-0.27], z_range=[0.0,0.18], num_pts=20000))
                    pcd_plane.paint_uniform_color([0,0,1])

                    open3d.visualization.draw_geometries([pcd, pcd_goal, pcd_plane, pcd_kidney])
        ...
        if state == "get shape servo plan":
            ...
            current_pc_numpy = down_sampling(get_partial_point_cloud("tissue"), num_pts=512)

            # if visualization:
            #     pcd = pcd_ize(current_pc_numpy)
            #     pcd.paint_uniform_color([0,0,0]) 
            #     open3d.visualization.draw_geometries([pcd, pcd_goal, pcd_kidney]) 
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if action is not None and gym.get_sim_time(sim) - closed_loop_start_time <= 8: 
                    ...
                    if visualization:
                        ...
                        pcd.paint_uniform_color([0,0,0])
                        open3d.visualization.draw_geometries([pcd, pcd_goal, pcd_kidney, pcd_plane])
