...
if __name__ == "__main__":
    ...
    if sim_type is gymapi.SIM_FLEX:
        ...
        sim_params.flex.num_outer_iterations = 8 #4
    ...
    soft_pose.p = gymapi.Vec3(0.0, -0.42, 0.02*0.5)
    ...
    data_recording_path = "/home/baothach/shape_servo_data/goal_generation/retraction/data"
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            if frame_count == 5:
                ...
                # plane_points = visualize_plane(constrain_plane, x_range=0.15, y_range=[-0.57,-0.27], z_range=0.1, num_pts=10000)
                ...
                # open3d.visualization.draw_geometries([pcd, pcd_plane, coor])
        ...
        if state == "generate preshape":                   
            ...
            if constrain_plane[0] > 0:
                target_pose[0] += -0.15*0.7/2   # shift from middle grasp point 
            elif constrain_plane[0] < 0:
                target_pose[0] += 0.15*0.7/2   # shift from middle grasp point
        ...
        if state == "get shape servo plan":
            ...
            # open3d.visualization.draw_geometries([pcd, pcd_goal, pcd_plane]) 
        ...
        if state == "move to goal":
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if action is not None and gym.get_sim_time(sim) - closed_loop_start_time <= 8: 
                    ...
                    success = check_success(constrain_plane, current_pc=get_partial_point_cloud(i))
                    execute_count += 1
                   
                    if not success:
                        rospy.logerr("Not succeed yet !!!")
                        if execute_count % 2 == 0:   # num times executing 1 goal shape (2 times)
                            delta += 0.02
                            goal_pc_numpy = get_goal_plane(constrain_plane=constrain_plane, initial_pc=initial_pc, delta=delta)
                            
                            if goal_pc_numpy is None:
                                rospy.logerr("Constrain plane moved outside the object :(. Resetting ...")
                                state = "reset"                                
                            else:
                                goal_pc_tensor = torch.from_numpy(goal_pc_numpy).permute(1,0).unsqueeze(0).float().to(device)
                                num_new_goal += 1
                        ...
                        for j, curr_trans in enumerate(curr_trans_on_trajectory):                        
                            ...
                            data = {"full pcs": full_pcs, "partial pcs": partial_pcs, "pos": p, "rot": R, "twist": twist, \
                                    "mani_point": mani_point, "constrain_plane": constrain_plane}
