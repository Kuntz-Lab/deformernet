...
if __name__ == "__main__":
    ...
    cylinder_pose.p = gymapi.Vec3(-0.02, 0.38-two_robot_offset, 0.04)
    ...
    for i in range(num_envs):
        ...
        cylinder_actor = gym.create_actor(env, cylinder_asset, cylinder_pose, "cylinder", i, 1, segmentationId=10)
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            if frame_count == 5:
                ...
                g_1_pos, g_2_pos = 0.35, -0.35
                
                pcd = pcd_ize(get_partial_point_cloud(i))
                # center, radius, slope = [-0.02,0.4-two_robot_offset], 0.02, 0.003
                center, radius, slope = [-0.02,0.38-two_robot_offset], 0.03, 0.006
                goal_spiral = generate_spiral(center, radius, slope, num_circle=1, z_shift=0.02, num_samples=10)
                pcd_goal = pcd_ize(goal_spiral)
        ...
        if state == "move to goal":
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if traj_index == len(plan_traj):
                    # frame_count += 1 
                    rospy.logerr("Success checkpoint!")
                    traj_index = 0
                    spiral_idx += 1
                    state = "get shape servo plan"           
