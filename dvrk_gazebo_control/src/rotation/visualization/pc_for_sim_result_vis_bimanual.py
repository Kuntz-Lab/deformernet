...
def init():
    for i in range(num_envs):
        ...
        davinci_dof_states['pos'][4] = 0.301
...
if __name__ == "__main__":
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            frame_count += 1
            gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka", "psm_main_insertion_joint"), 0.301)
            gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka2", "psm_main_insertion_joint"), 0.301)            
            if frame_count == 10:
        ...
        if state == "get shape servo plan":
            ...
            print("****** max_z", max_z)
            delta_x_1, delta_y_1, delta_z_1 = -0.01+0.05, max_y*0.4, 0.04 #max_z
            delta_x_2, delta_y_2, delta_z_2 = -0.01-0.05, max_y*0.4, 0.075 #max_z
            delta_alpha_1, delta_beta_1, delta_gamma_1 = -np.pi/300000, -np.pi/300000, -np.pi/3
            delta_alpha_2, delta_beta_2, delta_gamma_2 = -np.pi/300000, -np.pi/300000, -np.pi/3
        ...
        if state == "move to goal":
            ...
            if (not(20 in contacts or 21 in contacts) or not(9 in contacts or 10 in contacts)):  # lose contact w either robot 2 or robot 1    
                ...
                if (action_1 is not None) and (action_2 is not None) and gym.get_sim_time(sim) - closed_loop_start_time <= 8: 
                    ...
                    with open(os.path.join(goal_recording_path, "bimanual arm sample " + str(data_point_count) + ".pickle"), 'wb') as handle:
