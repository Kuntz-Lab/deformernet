...
if __name__ == "__main__":
    ...
    if sim_type is gymapi.SIM_FLEX:
        ...
        sim_params.flex.num_outer_iterations = 4#4 10
    ...
    model = DeformerNetBimanual().to(device)
    weight_path = f"/home/baothach/shape_servo_data/bimanual/multi_cylinders_1000Pa/weights/run1"        
    ...
    save_path = os.path.join(image_path, "recordings", "cylinder_vis_2")  
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            with torch.no_grad():
                ...
                pos = model(current_pc_tensor, goal_pc_tensor).detach().cpu().numpy() * 0.001
                alpha, beta, gamma = np.pi/30000, np.pi/30000, np.pi/30000
                rot_mat_1 = transformations.euler_matrix(alpha, beta, gamma)[:3,:3]
                rot_mat_2 = rot_mat_1
