...
if __name__ == "__main__":
    ...
    start_vis_cam = False#True #False
    vis_frame_count = 0
    num_image = 0
    save_path = os.path.join(main_path, "vis/6_goal")  
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            max_x = min(0.06, max_x)
            max_y = min(0.055, max_y)
            max_z = min(0.05, max_z)
            
            delta_x = np.random.choice([-max_x,max_x])
            ...
            delta_x, delta_y, delta_z = -0.065,0.06,0.05#max_z
            # delta_x, delta_y, delta_z = -0.065/1.5,0.06/1.2,0.05#max_z
        ...
        if  goal_point_count >= max_goal_count:                     
            ...
            with open(os.path.join(goal_recording_path, args.obj_name + "_test.pickle"), 'wb') as handle:
