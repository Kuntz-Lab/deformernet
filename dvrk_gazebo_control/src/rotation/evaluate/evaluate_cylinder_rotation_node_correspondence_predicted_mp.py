...
def get_mp_classifier(classifier_model, partial_init_pc, partial_goal_pc, full_init_pc, 
    ...
    # obb = full_pcd.get_oriented_bounding_cylinder()
    # center = full_pcd.get_center()    

    # # Get width, height, depth
    # points = np.asarray(obb.get_cylinder_points())
...
if __name__ == "__main__":
    ...
    if args.obj_type == "cylinder_1k":
        if no_rot:
            ...
            weight_path = "/home/baothach/shape_servo_data/generalization/multi_cylinders_1000Pa/weights/run1"
    ...
    #     weight_path = "/home/baothach/shape_servo_data/generalization/multi_cylinders_5kPa/weights/run3(partial)"
    ...
    if args.mp_method == "dense_predictor":
        ...
        weight_path = "/home/baothach/shape_servo_data/manipulation_points/multi_cylinders_1000Pa/weights/seg/run1"
        mp_seg_model.load_state_dict(torch.load(os.path.join(weight_path, "epoch 202")))        

        # weight_path = "/home/baothach/shape_servo_data/manipulation_points/multi_cylinders_1000Pa/weights/seg/run6"
        ...
        weight_path = "/home/baothach/shape_servo_data/manipulation_points/multi_cylinders_1000Pa/weights/classifier/run1"
    ...
    if args.inside:
        ...
        if args.no_rot:
            chamfer_recording_path = os.path.join(main_path, "chamfer_results", args.obj_type, "inside_gt_no_rot")
        else:
            chamfer_recording_path = os.path.join(main_path, "chamfer_results", args.obj_type, "inside_gt_w_rot")
    else:
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "generate preshape":                   
            rospy.loginfo("**Current state: " + state)
            # preshape_response = cylinderpcopy(preshape_response.palm_goal_pose_world[0].pose)        
            with torch.no_grad():
                ## Ground truth:
            ...
            target_pose = [-best_mp[0], -best_mp[1], best_mp[2] - ROBOT_Z_OFFSET-0.00, 0, 0.707107, 0.707107, 0]  # for heuristic
