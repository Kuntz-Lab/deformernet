...
if __name__ == "__main__":
    ...
    if sim_type is gymapi.SIM_FLEX:
        ...
        sim_params.flex.num_outer_iterations = 10#10#10
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "generate preshape":                   
            ...
            target_pose = [0, -cartesian_goal.position.y, cartesian_goal.position.z-ROBOT_Z_OFFSET-0.01,
                            0, 0.707107, 0.707107, 0]
        ...
        if state == "get shape servo plan":
            ...
            ####################################################
            # max_x = -1/4*h*1/2 #h/0.45 * 0.12 / 4  #min(0.06, max_x) 1/6 -> 1/3
            # max_y = 1/4*h*1/2 #h/0.40 * 0.08 / 1 #0.08 #min(0.055, max_y)
            max_z = h/0.45 * 0.05 #0.08 #min(0.05, max_z)
            
            # delta_x = max_x#np.random.choice([-max_x,max_x])
            delta_x = np.random.uniform(low = 1/6* h*1/2, high = 1/2.5* h*1/2)
            delta_x *= np.random.choice([-1,1])
            
            # delta_y = max_y#np.random.uniform(low = max_y/1.5, high = max_y)
            delta_y = np.random.uniform(low = 1/6* h*1/2, high = 1/2.5* h*1/2)
            ...
            if delta_x <= 0:
                ...
                delta_alpha = -np.random.uniform(low = 0*np.pi/4, high = np.pi/3)
                delta_beta = np.random.uniform(low = 0, high = np.pi/3)
            delta_alpha, delta_beta, delta_gamma = np.pi/3000000, -np.pi/3000000, np.pi/300000 
