...
if __name__ == "__main__":
    ...
    if args.inside:
        ...
        if args.no_rot:
            chamfer_recording_path = os.path.join(main_path, "chamfer_results", args.obj_type, "inside_no_rot_test")
        else:
            chamfer_recording_path = os.path.join(main_path, "chamfer_results", args.obj_type, "inside_w_rot_test")
    else:
    ...
    vis_frame_count = 0
    num_image = 0
    save_path = os.path.join(main_path, "vis/5")  
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if prepare_vis_cam:
            ...
            color = (0, 0, 255)
            thickness = 2 
            goal_xs, goal_ys = get_goal_projected_on_image(down_sampling(goal_datas[goal_count]["full pcs"][1], num_points=512), i, thickness = 0)
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if timeit.default_timer() - shapesrv_start_time >= max_shapesrv_time:
                    ...
                    if converge or chamfer_dist < min_chamfer_dist:
                        ...
                        final_node_distances.append(chamfer_dist) 
