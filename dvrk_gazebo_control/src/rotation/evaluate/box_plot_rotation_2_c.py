    ...
    for i in range_data:
        ...
        if os.path.isfile(file_name):
            ...
            # chamfer_data.extend([d if d < 900 else d-999 for d in data])
                ...
                data = pickle.load(handle)
            chamfer_data.extend(list(np.array(data["node"])/data["num_nodes"]*1000))
    ...
    for obj_type in ['1k']:
        ...
        for method_type in ['sample_ratio_']:     
            ...
            chamf_w_rot_ori = get_results(result_dir, prim_name, f"{prim_name}_{obj_type}", \
                                    inside=True, range_data=[k for k in range(6) if k not in exclude_list], method_type=f'{method_type}w_rot' )
            
            chamf_no_rot_ori = get_results(result_dir, prim_name, f"{prim_name}_{obj_type}", \
                                    inside=True, range_data=[k for k in range(6) if k not in exclude_list], method_type=f'{method_type}no_rot' )

            strange = get_results(result_dir, prim_name, f"{prim_name}_{obj_type}", \
                                    inside=True, range_data=[k for k in range(6) if k not in exclude_list], method_type=f'{method_type}w_rot_strange' )
            ...
            chamfers += list(chamf_w_rot) + list(chamf_no_rot) + list(strange)
            model_type += ["w orientation w MP input"]*len(chamf_w_rot) + ["w/o orientation w MP input"]*len(chamf_no_rot) + ["w orientation w/o MP input"]*len(strange)
            if method_type == '':
            ...
            elif method_type == "sample_ratio_":     
                categories += ["random boxes"]*(len(chamf_w_rot)+len(chamf_no_rot)+len(strange))
...
plt.title('Ablation study on box_1k using oracle MP', fontsize=16)
