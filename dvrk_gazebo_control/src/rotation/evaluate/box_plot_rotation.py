...
for obj_type in ['1k']:
    ...
    for method_type in ['']:    
        ...
        chamf_w_rot_ori = get_results(result_dir, prim_name, f"{prim_name}_{obj_type}", \
                                inside=True, range_data=[0,last_idx], method_type=f'{method_type}w_rot_long_box' )
        
        chamf_no_rot_ori = get_results(result_dir, prim_name, f"{prim_name}_{obj_type}", \
                                inside=True, range_data=[0,last_idx], method_type=f'{method_type}no_rot_long_box' )
...
ax=sns.boxplot(y="chamfer",x='category', hue='model type', data=df, whis=10, showfliers = True) #, whis=3.5
