...
if __name__ == "__main__":
    ...
    if sim_type is gymapi.SIM_FLEX:
        ...
        sim_params.flex.num_inner_iterations = 50#50
    ...
    asset_root = "/home/baothach/sim_data/Custom/Custom_urdf/multi_hemis_5kPa"
    ...
    rospy.logerr(f"Number of nodes: {get_point_cloud().shape[0]}")
    ...
    if args.data_category == "deformernet":
        data_recording_path = "/home/baothach/shape_servo_data/rotation_extension/multi_hemis_5kPa/data"
    elif args.data_category == "MP":
        data_recording_path = "/home/baothach/shape_servo_data/manipulation_points/multi_hemis_5kPa/mp_data" 
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "generate preshape":                   
            ...
            ee_loc_on_obj = abs(cartesian_goal.position.y-soft_pose.p.y) + object_length/2
        ...
        if state == "get shape servo plan":
            ...
            # max_x = max_y = max_z =  object_length/2 * 3/4    #r/0.2 * 0.08
            # max_z *= 3/4
            # print("actual max:", max_x)
            ...
            ### Change to collect data with or without orientation
            ...
            if cartesian_goal.position.x >= 0:
            ...
            # delta_x, delta_y, delta_z = -max_x if cartesian_goal.position.x >= 0 else max_x, max_y, max_z
            # delta_alpha, delta_beta, delta_gamma = np.pi/300000, np.pi/300000, np.pi/300000
            ...
            tvc_behavior = TaskVelocityControl2([x,y,z,alpha,beta,gamma], robot, sim_params.dt, 3, vel_limits=vel_limits, pos_threshold = 2e-3, ori_threshold=5e-2)
        ...
        if state == "move to goal":
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if frame_count % 15 == 0: #15
                    ...
                    if args.data_category == "MP":              
                        if frame_count == 0:
                            mani_point = deepcopy(gym.get_actor_rigid_body_states(envs[i], kuka_handles_2[i], gymapi.STATE_POS)[-3])
                ...
                if action is not None and gym.get_sim_time(sim) - closed_loop_start_time <= 8: 
                    ...
                    if args.data_category == "deformernet":
                        ...
                        for j in range(len(pc_on_trajectory)):    
                            ...
                            data = {"full pcs": full_pcs, "partial pcs": partial_pcs, "mani_point": mani_point["pose"], "obj_name": args.obj_name}
                            with open(os.path.join(data_recording_path, "sample " + str(data_point_count) + ".pickle"), 'wb') as handle:
                                pickle.dump(data, handle, protocol=pickle.HIGHEST_PROTOCOL)                          
