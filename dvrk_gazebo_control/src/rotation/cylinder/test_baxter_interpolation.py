...
from interpolation_baxter_utils import *
...
if __name__ == "__main__":
    ...
    r = 0.02 #r = data[args.obj_name]["radius"]
    h = 0.4  #h = data[args.obj_name]["height"]
    ...
    # asset_root = "/home/baothach/sim_data/Custom/Custom_urdf/multi_cylinders_official"
    ...
    data_recording_path = "/home/baothach/shape_servo_data/rotation_extension/multi_cylinder_10kPa/single_thin_cylinder_data_on_ground"
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka2", "psm_main_insertion_joint"), 0.25)            
            if frame_count == 10:
        ...
        if state == "grasp object":             
            ...
            if dof_states['pos'][8] < 0.35:
                ...
                initial_pose = np.array([0.08,0.04,0.02, np.pi/3000000, np.pi/3000000, np.pi/2])
                goal_pose = np.array([-0.08,0.04,0.02, np.pi/3000000, -np.pi/3000000, -np.pi/2])
                cps = compute_goal(initial_pose, goal_pose, num_cp=2, fractions=[8/10, 9.5/10])                
                for cp in cps:
                    print(cp)
                    print("==============")  
        ...
        if state == "get shape servo plan":
            ...
            # delta_z = 0 #np.random.uniform(low = 0.03, high = max_z)      

            if sample_count == 0:
                delta_alpha, delta_beta, delta_gamma = np.pi/3000000, np.pi/3000000, np.pi/4
                delta_x, delta_y, delta_z = 0.04,0.04,0.02
            else:
                # delta_alpha, delta_beta, delta_gamma = np.pi/3000000, -np.pi/3000000, -np.pi/2
                # delta_x, delta_y, delta_z = -0.08,0.00,0.0
                delta_x, delta_y, delta_z = cps[cp_count][0], cps[cp_count][1], cps[cp_count][2]
                delta_alpha, delta_beta, delta_gamma = cps[cp_count][3], cps[cp_count][4], cps[cp_count][5]
                cp_count += 1
