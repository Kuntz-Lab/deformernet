...
if __name__ == "__main__":
    ...
    asset_options.thickness = 0.001 #0.0005#0.0001
    ...
    soft_asset_file = 'cylinder_2_attached.urdf'  # height = 0.2 m   
    ...
    cam_positions.append(gymapi.Vec3(0.1, -0.62, 0.2))
    ...
    goal_datas = []
    goal_point_count = 0
    max_goal_count = goal_point_count+1#10
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            delta_alpha, delta_beta, delta_gamma = 0*np.pi/3, 0*np.pi/3, np.pi/3
            delta_x, delta_y, delta_z = 0.1,0.04,0.02
        ...
        if state == "move to goal":
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if action is not None and gym.get_sim_time(sim) - closed_loop_start_time <= 5: 
                    ...
                    # ys = full_pc_init[:,1]
                    # rospy.logwarn(f"min y: {min(ys) }; and max y: {max(ys)}")
                    pcd = open3d.geometry.PointCloud()
                    pcd.points = open3d.utility.Vector3dVector(partial_goal_pcs[0])   
                    pcd.paint_uniform_color([0,0,0])
                    pcd_goal = open3d.geometry.PointCloud()
                    pcd_goal.points = open3d.utility.Vector3dVector(partial_goal_pcs[1])   
        ...
        if  goal_point_count >= max_goal_count:                     
            ...
            with open(os.path.join(goal_recording_path, f"cylinder_{data_point_count}.pickle"), 'wb') as handle:
