...
for (prim_name, stiffness, inside) in list(product(prim_names, stiffnesses, inside_options)):
    ...
    for mp_method in mp_methods:       
        ...
        # if f"{prim_name}_{stiffness}" in ["hemis_1k"] and mp_method == "keypoint":
        #     res, res_avg = get_results(prim_name, stiffness, inside, "classifier", use_rot, use_mp_input, range_data=range_data)
        #     res_avg = res_avg + 0.08
        #     res = res + 0.04
