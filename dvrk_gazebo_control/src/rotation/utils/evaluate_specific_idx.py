...
def run_collect_goals_loop(headless, prim_name, stiffness, inside, num_obj=10):
    for i in range(0, num_obj):
        ...
        os.system(f"rosrun dvrk_gazebo_control collect_goals_{prim_name}_rotation.py --flex --headless {str(headless)} --obj_name {i}\
...
mp_methods = ["dense_predictor"] # ["ground_truth", "classifier", "keypoint"] #["ground_truth", "dense_predictor", "classifier", "keypoint"]
