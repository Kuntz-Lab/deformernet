...
for (prim_name, stiffness, inside) in list(product(prim_names, stiffnesses, inside_options)):
    ...
    # model_type += ["Our DeformerNet"]*filtered_idxs.shape[0] + ["DeformerNet using oracle MP"]*filtered_idxs.shape[0] \
    #             + ["DeformerNet using MP classifier"]*filtered_idxs.shape[0] + ["DeformerNet using MP heuristics"]*filtered_idxs.shape[0] 
