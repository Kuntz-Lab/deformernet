...
if __name__ == "__main__":
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            # delta_z = np.random.uniform(low = 0.02, high = max_z)    
            ...
            # delta_alpha = np.random.choice([-np.pi/3,np.pi/3])
            # delta_beta = np.random.choice([-np.pi/3,np.pi/3])
            # delta_gamma = np.random.choice([-np.pi/2,np.pi/2])
            # delta_alpha, delta_beta, delta_gamma = np.pi/3000000, np.pi/3000000,np.pi/3000000


            delta_x, delta_y, delta_z = 0.07,0.05,0.04  # 0, 13
            # delta_x, delta_y, delta_z = 0.00,0.00,0.00  # 0, 13
            delta_alpha, delta_beta, delta_gamma = -np.pi/3,np.pi/3,np.pi/2

            # delta_x, delta_y, delta_z = 0.04,0.04,0.04  # 100, 101
            # delta_alpha, delta_beta, delta_gamma = -np.pi/3, np.pi/3,np.pi/3
            # delta_x, delta_y, delta_z = 0.00,0.00,0.02  # 102
            # delta_alpha, delta_beta, delta_gamma = -np.pi/30000,-np.pi/30000,np.pi/2
            # delta_x, delta_y, delta_z = 0.00,0.00,0.00  # 103
            # delta_alpha, delta_beta, delta_gamma = -np.pi/30000,-np.pi/3,np.pi/20000
            # delta_x, delta_y, delta_z = -0.03,0.00,0.03  # 104
            # delta_alpha, delta_beta, delta_gamma = -np.pi/30000,np.pi/30000,-np.pi/2
            # delta_x, delta_y, delta_z = 0.02,0.03,0.02  # 105
            # delta_alpha, delta_beta, delta_gamma = -np.pi/30000,np.pi/3,np.pi/3
            # delta_x, delta_y, delta_z = 0.00,0.01,0.03  # 106
            # delta_alpha, delta_beta, delta_gamma = -np.pi/30000,np.pi/30000,np.pi/30000
            # delta_x, delta_y, delta_z = 0.03,0.03,0.03  # 107
            # delta_alpha, delta_beta, delta_gamma = np.pi/300000, np.pi/300000,np.pi/200000
            # delta_x, delta_y, delta_z = 0.06,0.04,0.06  # 14
            # delta_alpha, delta_beta, delta_gamma = -np.pi/300000, np.pi/300000,np.pi/300000
            # delta_x, delta_y, delta_z = 0.07,0.06,0.05  # 0
            # delta_alpha, delta_beta, delta_gamma = -np.pi/3, np.pi/3,np.pi/3
            ...
            # print(" ++++ new pose:", delta_x, delta_y, delta_z, delta_alpha, delta_beta, delta_gamma)
            
            # tvc_behavior = TaskVelocityControl2([delta_x, delta_y, delta_z], robot, sim_params.dt, 3, vel_limits=vel_limits)
            tvc_behavior = TaskVelocityControl2([x,y,z,alpha,beta,gamma], robot, sim_params.dt, 3, vel_limits=vel_limits, pos_threshold = 1e-3, ori_threshold=7e-2)
        ...
        if state == "move to goal":
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                frame_count += 1   
                # print("count:", random_count)        
                
                # action, data_pt = tvc_behavior.get_action(debug=True)  
                # data.append(data_pt)                
                action = tvc_behavior.get_action()  
                if action is not None and gym.get_sim_time(sim) - closed_loop_start_time <= 10: 
                    ...
                    data = {"full pc": full_pc_goal, "partial pc": pc_goal, "pos": p, "rot": R, "mani_point": mani_point, "obj_name": args.obj_name}
