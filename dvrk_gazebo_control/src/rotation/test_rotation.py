...
def init():
    for i in range(num_envs):
        ...
        # davinci_dof_states['pos'][8] = 0.35#1.5
        # davinci_dof_states['pos'][9] = -0.35#0.8
        davinci_dof_states['pos'][8] = 1.5
        davinci_dof_states['pos'][9] = 0.8   
        davinci_dof_states['pos'][4] = 0.1     
...
if __name__ == "__main__":
    ...
    # asset_root = "/home/baothach/sim_data/Custom/Custom_urdf/cylinder"
    # soft_asset_file = "cylinder_1.urdf"


    soft_pose = gymapi.Transform()
    # soft_pose.p = gymapi.Vec3(0.0, -0.42+two_robot_offset, 0.01818)
    soft_pose.p = gymapi.Vec3(0.0, -0.42+two_robot_offset, 0.01)
    soft_pose.r = gymapi.Quat(0.0, 0.0, -0.707107, 0.707107)
    # soft_pose.p = gymapi.Vec3(0.0, -0.42+two_robot_offset, 0.03)
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "generate preshape":                   
            ...
            # target_pose = [cartesian_goal.position.x, cartesian_goal.position.y, 0.1-ROBOT_Z_OFFSET,
            #                 0, 0.707107, 0.707107, 0]
            target_pose = [cartesian_goal.position.x, cartesian_goal.position.y, cartesian_goal.position.z-ROBOT_Z_OFFSET-0.02,
                            0, 0.707107, 0.707107, 0]

            mtp_behavior = MoveToPose(target_pose, robot, sim_params.dt, 1, open_gripper=True)
        ...
        if state == "grasp object":             
            ...
            if dof_states['pos'][8] < 0.35:
                ...
                dof_props_2["damping"][:8].fill(400.0)
                gym.set_actor_dof_properties(env, kuka_handles_2[i], dof_props_2)
        ...
        if state == "get shape servo plan":
            ...
            # delta_x, delta_y, delta_z = 0.1,0,0.03
            # delta_x, delta_y, delta_z = 0.0,0.0,0.0
            # delta_alpha, delta_beta, delta_gamma = 0.785,0.785,0.785
            # delta_alpha, delta_beta, delta_gamma = 1.56,1.56,1.56
            # delta_alpha, delta_beta, delta_gamma = 0.0,0.0,0.0
            # delta_alpha, delta_beta, delta_gamma = 0.0,0,0.785      
            # (nhin doi dien voi robot): pitch, yaw, roll
            # delta_alpha, delta_beta, delta_gamma = np.pi/2,np.pi/3,np.pi/2
            # delta_x, delta_y, delta_z = 0.00,0.00,0.04
            # delta_alpha, delta_beta, delta_gamma = -np.pi/3, 0, 0
            delta_x, delta_y, delta_z = 0.05,0.04,0.04  # 12, 13
        ...
        if state == "move to goal":
            ...
            if False:
                ...
                if action is not None and gym.get_sim_time(sim) - closed_loop_start_time <= 100: 
