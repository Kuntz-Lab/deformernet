...
if __name__ == "__main__":
    ...
    if meta_model:
        ...
        weight_path = "/home/baothach/shape_servo_data/rotation_extension/box/weights/mp/run1/"
        model.load_state_dict(torch.load(weight_path + "epoch " + str(118)))
    ...
    model_no_rot.load_state_dict(torch.load(os.path.join(weight_path, "run1epoch 300")))  
    model_no_rot.eval()

    

    # Get goal pc:
    with open(os.path.join(data_recording_path, "sample " + str(12) + ".pickle"), 'rb') as handle:
        data = pickle.load(handle)
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            if set_init_pose and with_init_pose:
                ...
                delta_alpha, delta_beta, delta_gamma = np.pi/3000000,np.pi/300000,-np.pi/30000000
                alpha = delta_alpha + init_eulers[0]
                beta = delta_beta + init_eulers[1]
                gamma = delta_gamma + init_eulers[2]
                ...
                # delta_alpha, delta_beta, delta_gamma = transformations.euler_from_matrix(temp1)
                # alpha = delta_alpha + init_eulers[0]
                # beta = delta_beta + init_eulers[1]
                # gamma = delta_gamma + init_eulers[2]
                desired_pos = np.clip(desired_pos, a_min = init_pose[:3,3]-0.5, a_max=init_pose[:3,3]+0.5)
                
                # rot_lims = np.array([500,500,500])
                # desired_rot = np.clip([alpha,beta,gamma], a_min = np.array(init_eulers)-rot_lims, a_max=np.array(init_eulers)+rot_lims)
                # desired_rot = np.clip([alpha,beta,gamma], a_min = np.array(anchor_eulers)-rot_lims, a_max=np.array(anchor_eulers)+rot_lims)
                # tvc_behavior = TaskVelocityControl2([*desired_pos,*desired_rot], robot, sim_params.dt, 3, vel_limits=vel_limits, pos_threshold = 1e-3, ori_threshold=5e-2)
        ...
        if state == "move to goal":           
            ...
            if action is None or gym.get_sim_time(sim) - closed_loop_start_time >= 7:   
                ...
                if vis:
                    if dual_vis:
                        open3d.visualization.draw_geometries([pcd, pcd_goal, pcd_goalx]) 
                    else:
