...
if __name__ == "__main__":
    ...
    main_path = f"/home/baothach/shape_servo_data/rotation_extension/bimanual/multi_{args.obj_type}Pa/evaluate_occlusion"
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            # delta_x_1 = random.uniform(*random.choice([(-max_x, -max_x/2), (max_x/2, max_x)]))
            # delta_y_1 = np.random.uniform(low = 0.0, high = max_y)
            delta_z_1 = np.random.uniform(low = max_z/2, high = max_z) 
            # delta_x_2 = random.uniform(*random.choice([(-max_x, -max_x/2), (max_x/2, max_x)]))
            # delta_y_2 = np.random.uniform(low = 0.0, high = max_y)
            ...
            # delta_x_1, delta_y_1, delta_z_1 = max_x, 0, max_z
            # delta_x_2, delta_y_2, delta_z_2 = -max_x, 0, max_z
        ...
        if state == "move to goal":
            ...
            if (not(20 in contacts or 21 in contacts) or not(9 in contacts or 10 in contacts)):  # lose contact w either robot 2 or robot 1    
                ...
                if (action_1 is not None) and (action_2 is not None) and gym.get_sim_time(sim) - closed_loop_start_time <= 8: 
                    ...
                    with open(os.path.join(goal_recording_path, f"{args.prim_name}_{data_point_count}.pickle"), 'wb') as handle:
                    ...
                    curr_trans_on_trajectory_1 = []  
                    curr_trans_on_trajectory_2 = []
                    state = "reset" #"get shape servo plan"
