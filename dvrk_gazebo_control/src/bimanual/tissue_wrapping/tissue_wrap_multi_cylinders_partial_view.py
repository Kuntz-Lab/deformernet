...
if __name__ == "__main__":
    ...
    shift_x = np.array([0]) #np.random.uniform(low = -0.05, high = 0.05, size=1) 
    shift_y = np.array([0]) #np.random.uniform(low = -0.05, high = 0.05, size=1) 
    shift_z = np.array([0]) #np.random.uniform(low = 0, high = 0.05, size=1) 
    cylinder_shift = np.concatenate((shift_x, shift_y, shift_z))
    print("****cylinder_shift:", cylinder_shift)
    
    
    cylinder_pose.p = gymapi.Vec3(cylinder_shift[0], -two_robot_offset/2+cylinder_shift[1], cylinder_shift[2]+0.04) #np.array([0.03,-0.02,0.06])
    ...
    for i in range(num_envs):
        ...
        envs_cylinder = gym.create_env(sim, env_lower, env_upper, num_per_row)   
        
        soft_actor = gym.create_actor(env_obj, soft_asset, soft_pose, "soft", i, 0)
        object_handles.append(soft_actor)

        cylinder_actor = gym.create_actor(envs_cylinder, cylinder_asset, cylinder_pose, "cylinder", i+2, 1, segmentationId=11)
        color = gymapi.Vec3(1,0,0)
        gym.set_rigid_body_color(envs_cylinder, cylinder_actor, 0, gymapi.MESH_VISUAL_AND_COLLISION, color)
    ...
    # cam_positions.append(gymapi.Vec3(0.3, cylinder_pose.p.y-0.3, cylinder_pose.p.z + 0.15))
    # cam_targets.append(gymapi.Vec3(0.0, cylinder_pose.p.y, cylinder_pose.p.z))  

    cam_positions.append(gymapi.Vec3(0.17, cylinder_pose.p.y-0.1, 0.2))
    cam_targets.append(gymapi.Vec3(0.0, cylinder_pose.p.y, 0.01)) 
    ...
    mp_seg_model = ManiPointSegment(num_classes=3).to(device)
    weight_path = "/home/baothach/shape_servo_data/manipulation_points/bimanual/multi_boxes_1000Pa/weights/run1"
    mp_seg_model.load_state_dict(torch.load(os.path.join(weight_path, "epoch 104")))    
    mp_seg_model.eval()


    goal_pc_numpy = get_cylinder_goal_pc(radius=(0.16*0.7)/(2*np.pi), height=0.15*0.7, translation=[0,-two_robot_offset/2, 0.9*0.04]) # circumfrence = tissue length
    ...
    image_save_path = "/home/baothach/shape_servo_data/tissue_wrap/random_pics"  
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            frame_count += 1
            gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka", "psm_main_insertion_joint"), 0.26)
            gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka2", "psm_main_insertion_joint"), 0.26)            
            if frame_count == 10:
        ...
        #     current_pc = get_point_cloud()     
        #     print("distance 2 edges:", np.mean(current_pc[edges_idxs_1], axis=0)-np.mean(current_pc[edges_idxs_2], axis=0))       


        if state == "generate preshape":                   
            rospy.loginfo("**Current state: " + state)
            
            current_pc = down_sampling(get_partial_point_cloud(i)) + np.array([0,0,0.02])
        ...
        if state == "get shape servo plan":
            ...
            gym.render_all_camera_sensors(sim)
            im = gym.get_camera_image(sim, envs_obj[i], cam_handles[0], gymapi.IMAGE_COLOR).reshape((cam_height,cam_width,4))[:,:,:3]
            path =  os.path.join(image_save_path, f'abc.png')                  
            cv2.imwrite(path, im)
            ...
            if move_to_centroid:      
                ...
                # if random_count_2 == 0:
                #     max_ys = max(current_pc[:,1])
                #     min_ys = min(current_pc[:,1])
                #     avg = (max_ys+min_ys)/2
                #     # current_pc = np.array([point for point in current_pc if point[1]>=min(ys)+0.03])
                #     # current_pc = np.array([point for point in current_pc if point[1]<=avg or point[1] >= avg + 0.03])
                #     current_pc = np.array([point for point in current_pc if point[1]<=min(ys)+0.05 or point[1] >= min(ys)+0.07])
                #     random_count_2 += 1



                current_pc = down_sampling(current_pc)
                current_pc_tensor = torch.from_numpy(current_pc).permute(1,0).unsqueeze(0).float().to(device)                       
                ...
                output = mp_seg_model(current_pc_tensor, goal_pc_tensor)
                success_probs_1 = np.exp(output.squeeze().cpu().detach().numpy())[1,:]
                success_probs_2 = np.exp(output.squeeze().cpu().detach().numpy())[2,:]
                best_mp_1 = current_pc[np.argmax(success_probs_1)]
                best_mp_2 = current_pc[np.argmax(success_probs_2)]
                mani_point_1_sphere = open3d.geometry.TriangleMesh.create_sphere(radius=0.01)
                mani_point_1_sphere.paint_uniform_color([0,0,1])
                mani_point_1_sphere.translate(tuple(best_mp_1))
                mani_point_2_sphere = open3d.geometry.TriangleMesh.create_sphere(radius=0.01)
                mani_point_2_sphere.paint_uniform_color([1,0,0])
                mani_point_2_sphere.translate(tuple(best_mp_2))

                print("++++++++++")
                print(max(success_probs_1), min(success_probs_1), np.mean(success_probs_1))

                heats = np.array([[prob, 0, 0] for prob in success_probs_1/max(success_probs_1)])
                pcd.colors =  open3d.utility.Vector3dVector(heats) 
                
                pcd_2 = deepcopy(pcd)
                heats = np.array([[prob, 0, 0] for prob in success_probs_2/max(success_probs_2)])
                pcd_2.colors =  open3d.utility.Vector3dVector(heats) 
                

                open3d.visualization.draw_geometries([pcd, pcd_2.translate((0.2,0,0)), pcd_goal.paint_uniform_color([0,0,1]), \
                ...
                #     # # desired_position[2] += 0.03
                #     # # desired_position[5] += 0.03

                #     desired_position[2] = max(desired_position[2], desired_position[5]) + 0.03
                #     desired_position[5] = desired_position[2]
                #     # desired_position[1] = 0.05
                #     # desired_position[4] = 0.055
