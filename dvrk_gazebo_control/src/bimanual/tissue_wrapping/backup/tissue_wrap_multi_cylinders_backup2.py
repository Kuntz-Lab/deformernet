...
if __name__ == "__main__":
    ...
    goal_pc_numpy = get_cylinder_goal_pc(radius=(0.16*0.7)/(2*np.pi), height=0.15*0.7, translation=[0,-two_robot_offset/2, 1.0*0.04+0.03]) # circumfrence = tissue length
        ...
        if state == "move to goal":
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if timeit.default_timer() - shapesrv_start_time >= max_shapesrv_time:
                    ...
                    condition_2 = percent >= 0.95 and iter_count > 5
