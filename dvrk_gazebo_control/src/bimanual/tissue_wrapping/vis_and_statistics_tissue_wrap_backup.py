...
#     percent = compute_intersection_percent(backbone, data["cylinder_shift"], vis = False)
...
# with open("saved_percents.pickle", 'wb') as handle:
#     pickle.dump(intersection_percents, handle, protocol=pickle.HIGHEST_PROTOCOL)  


# with open("saved_distances.pickle", 'wb') as handle:
...
### Visualize
with open("saved_percents.pickle", 'rb') as handle:
    intersection_percents = pickle.load(handle)   

with open("saved_distances.pickle", 'rb') as handle:
    distances = pickle.load(handle)

sorted_idxs = np.argsort(intersection_percents)

print("average intersection percent:", np.mean(distances))
print("min intersection percent:", np.min(distances), np.argmin(intersection_percents))
print("max intersection percent:", np.max(distances), np.argmax(intersection_percents))
