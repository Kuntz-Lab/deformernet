...
if __name__ == "__main__":
    ...
    for i in range(num_envs):
        ...
        temp_env = env
        cylinder_actor = gym.create_actor(temp_env, cylinder_asset, cylinder_pose, "cylinder", i, 1, segmentationId=11)
        color = gymapi.Vec3(1,0,0)
        gym.set_rigid_body_color(temp_env, cylinder_actor, 0, gymapi.MESH_VISUAL_AND_COLLISION, color)
    ...
    # goal_pc_numpy = get_cylinder_goal_pc(radius=(0.3*0.7)/(2*np.pi), height=0.1*0.8, translation=[0,-two_robot_offset/2, 1.0*0.04+0.03]) # circumfrence = tissue length
    # goal_pc_numpy = get_cylinder_goal_pc(radius=(0.16*0.7)/(2*np.pi), height=0.15*0.7, translation=[0.00,-two_robot_offset/2, 1.0*0.04+0.03]) # circumfrence = tissue length
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            if frame_count == 10:
                ...
                # ys = current_pc[:,1]
                # xs = current_pc[:,0]
                # print("***** dim h, w:", (max(xs)-min(xs))/0.8, (max(ys)-min(ys))/0.8)  #0.15 0.20
