...
if __name__ == "__main__":
    ...
    if random_movement:
        pos_2 = np.random.uniform(low = -0.05, high = 0.05)
        pos_1 = np.random.uniform(low = -0.05, high = 0.05)
        rot_angle_2 = np.random.uniform(low = -np.pi/6, high = np.pi/6)
        rot_angle_1 = np.random.uniform(low = -np.pi/6, high = np.pi/6)
        ...
        pos_1 = pos_2 = np.random.uniform(low = -0.05, high = 0.05)
        rot_angle_2 = 0
        rot_angle_1 = 0             
    ...
    quat = transformations.quaternion_from_euler(*[rot_angle_2,0,np.pi/2])
    soft_pose_2.r = gymapi.Quat(*quat)

  
    quat = transformations.quaternion_from_euler(*[np.pi + rot_angle_1, 0, np.pi/2])
    ...
    data_recording_path = "/home/baothach/shape_servo_data/tube_connect/cylinder_rot/data"
    terminate_count = 0
    sample_count = 0
    frame_count = 0
    group_count = 0
    ...
    max_sample_count = 3 if random_movement else 2
    max_data_point_count = 15000
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            if random_movement:
                grasp_pt_1 = deepcopy(gym.get_actor_rigid_body_states(envs[i], kuka_handles[i], gymapi.STATE_POS)[-3])["pose"]["p"]["y"]
                grasp_pt_2 = deepcopy(gym.get_actor_rigid_body_states(envs[i], kuka_handles_2[i], gymapi.STATE_POS)[-3])["pose"]["p"]["y"]
                d_1 = 0.4/(2*2) - abs(grasp_pt_1 - soft_pose_1.p.y)
                d_2 = 0.4/(2*2) - abs(grasp_pt_2 - soft_pose_2.p.y)

                max_x = 0.1
                min_y, max_y = 0.06, 0.03 
                max_z = 0.1

                delta_x_1 = np.random.uniform(low = -max_x, high = max_x)
                delta_y_1 = np.random.uniform(low = -min_y, high = max_y) 
                delta_z_1 = np.random.uniform(low = 0.02, high = max_z)     
                ...
                x_1 = delta_x_1 
                y_1 = delta_y_1 + two_robot_offset/2 - d_1
                ...
                tvc_behavior_1 = TaskVelocityControl2([x_1,y_1,z_1,alpha_1,beta_1,gamma_1], robot_1, sim_params.dt, 3, vel_limits=vel_limits, pos_threshold = 1e-3, ori_threshold=5e-2)

                delta_x_2 = np.random.uniform(low = -max_x, high = max_x)
                delta_y_2 = np.random.uniform(low = -min_y, high = max_y) 
                delta_z_2 = np.random.uniform(low = 0.02, high = max_z)     
                ...
                # print("delta_x_1, delta_y_1, delta_z_1, delta_x_2, delta_y_2, delta_z_2:", \
                #         delta_x_1, delta_y_1-init_pose_1[1,3], delta_z_1, delta_x_2, delta_y_2-init_pose_2[1,3], delta_z_2)


                x_2 = delta_x_2 
                y_2 = delta_y_2 + two_robot_offset/2 - d_2 
                ...
                tvc_behavior_2 = TaskVelocityControl2([x_2,y_2,z_2,alpha_2,beta_2,gamma_2], robot_2, sim_params.dt, 3, vel_limits=vel_limits, pos_threshold = 1e-3, ori_threshold=5e-2)
            
            else:
            
                if not maintain_alignment:
                    max_x = 0.1
                    # max_y = 0.1 
                    max_z = 0.03

                    delta_x_1 = np.random.uniform(low = -max_x, high = max_x)
                    delta_y_1 = np.random.uniform(low = two_robot_offset/3.5 + abs(abs(current_pose_1["y"]) - 2.15*two_robot_offset/3.5), \
                                                        high = init_pose_1[1,3]) 
                    delta_z_1 = np.random.uniform(low = 0.00, high = max_z)     
                    delta_alpha_1 = 0 #np.random.uniform(low = -np.pi/3, high = np.pi/3)
                    delta_beta_1 = 0 #np.random.uniform(low = -np.pi/3, high = np.pi/3) 
                    delta_gamma_1 = 0.0000001 #np.random.uniform(low = -np.pi/3, high = np.pi/3)     
                    ...
                    delta_x_2 = np.random.uniform(low = -max_x, high = max_x)
                    delta_y_2 = np.random.uniform(low = two_robot_offset/3.5 + abs(abs(current_pose_2["y"]) - 1.35*two_robot_offset/3.5), \
                                                    high = init_pose_2[1,3])
                    delta_z_2 = np.random.uniform(low = 0.00, high = max_z)     
                    delta_alpha_2 = 0 #np.random.uniform(low = -np.pi/3, high = np.pi/3)
                    delta_beta_2 = 0 #np.random.uniform(low = -np.pi/3, high = np.pi/3) 
                    delta_gamma_2 = 0.0000001 #np.random.uniform(low = -np.pi/3, high = np.pi/3)     
                    # print("delta_x_1, delta_y_1, delta_z_1, delta_x_2, delta_y_2, delta_z_2:", \
                    #         delta_x_1, delta_y_1-init_pose_1[1,3], delta_z_1, delta_x_2, delta_y_2-init_pose_2[1,3], delta_z_2)


                    x_2 = delta_x_2 
                    y_2 = delta_y_2 #+ init_pose_1[1,3] 
                    z_2 = delta_z_2 + init_pose_2[2,3]
                    alpha_2 = delta_alpha_2 + init_eulers_2[0]
                    beta_2 = delta_beta_2 + init_eulers_2[1]
                    gamma_2 = delta_gamma_2 + init_eulers_2[2]
                    ...
                    strategy = random.choice(["zero_psi", "zero_zeta", "mixed"]) 
                    rospy.logwarn(f"strategy: {strategy}")
                    add_random_increments = random.choice([True, False, False])                 

                    if strategy == "zero_psi":
                        psi = 0
                        zeta = np.random.uniform(low = -np.pi/4, high = np.pi/4)
                        z_up = np.random.uniform(low = 0.0, high = 0.07)
                    elif strategy == "zero_zeta":
                        zeta = 0
                        psi = np.random.uniform(low = 0, high = np.pi/3)  
                        z_up = np.random.uniform(low = 0.04, high = 0.1)  
                    else:            
                        psi = np.random.uniform(low = 0, high = np.pi/3)
                        zeta = np.random.uniform(low = -np.pi/4, high = np.pi/4)
                        z_up = np.random.uniform(low = 0.04, high = 0.1) 
                    
                    if add_random_increments:
                        icrements = [np.random.uniform(low = -0.015, high = 0.015),
                                    np.random.uniform(low = -0.01, high = 0.015),
                                    np.random.uniform(low = -0.02, high = 0.02)]
                        rospy.logwarn(f"add icrements: {icrements}")
                    
                    # z_up = 0.25 #0.04
                    # print("z_up:", z_up)
                    grasp_pt_1 = deepcopy(gym.get_actor_rigid_body_states(envs[i], kuka_handles[i], gymapi.STATE_POS)[-3])["pose"]["p"]["y"]
                    grasp_pt_2 = deepcopy(gym.get_actor_rigid_body_states(envs[i], kuka_handles_2[i], gymapi.STATE_POS)[-3])["pose"]["p"]["y"]
                    if strategy == "zero_zeta" or  strategy == "mixed":
                        d_1 = 0.4/(2*2) - abs(grasp_pt_1 - soft_pose_1.p.y)
                        d_2 = 0.4/(2*2) - abs(grasp_pt_2 - soft_pose_2.p.y)
                        if d_1 < d_2:
                            psi *= -1
                    ...
                    d = 0.4/(2*2) - abs(grasp_pt_1 - soft_pose_1.p.y)  
                    ...
                    psi /= 1.5
                    delta_x_1, delta_y_1, delta_z_1 = d*np.sin(zeta),d-d*np.cos(zeta)+(((2*d+m)/np.cos(psi)-2*d)/2-m/2)*np.cos(psi),t*np.sin(psi) + z_up  
                    # if add_random_increments:
                    #     delta_x_1, delta_y_1, delta_z_1 = increment(delta_x_1, delta_y_1, delta_z_1, deltas=icrements)
                    ...
                    tvc_behavior_1 = TaskVelocityControl2([x_1,y_1,z_1,alpha,beta,gamma], robot_1, sim_params.dt, 3, vel_limits=vel_limits, pos_threshold = 1e-3, ori_threshold=5e-2)



                    # grasp_pt_2 = deepcopy(gym.get_actor_rigid_body_states(envs[i], kuka_handles_2[i], gymapi.STATE_POS)[-3])["pose"]["p"]["y"]
                    d = 0.4/(2*2) - abs(grasp_pt_2 - soft_pose_2.p.y)
                    
                    t = d + m/2
                    delta_x_2, delta_y_2, delta_z_2 = d*np.sin(zeta),d-d*np.cos(zeta)+(((2*d+m)/np.cos(psi)-2*d)/2-m/2)*np.cos(psi),-t*np.sin(psi) + z_up 
                    if add_random_increments:
                        delta_x_2, delta_y_2, delta_z_2 = increment(delta_x_2, delta_y_2, delta_z_2, deltas=icrements)
                    ...
                    # print("delta_x_1, delta_y_1, delta_z_1, delta_x_2, delta_y_2, delta_z_2:", delta_x_1, delta_y_1, delta_z_1, delta_x_2, delta_y_2, delta_z_2)

                    tvc_behavior_2 = TaskVelocityControl2([x_2,y_2,z_2,alpha,beta,gamma], robot_2, sim_params.dt, 3, vel_limits=vel_limits, pos_threshold = 1e-3, ori_threshold=5e-2)
        ...
        if state == "move to goal":
            ...
            if (not(20 in contacts or 21 in contacts) or not(9 in contacts or 10 in contacts)):  # lose contact w either robot 2 or robot 1    
                ...
                if not(action_1 is None or action_2 is None) and gym.get_sim_time(sim) - closed_loop_start_time <= 4:   
                    ...
                    if random_movement or maintain_alignment:                    
                        ...
                        for j, (curr_trans_1, curr_trans_2) in enumerate(zip(curr_trans_on_trajectory_1, curr_trans_on_trajectory_2)):
                            ...
                            data = {"full pcs": full_pcs, "partial pcs": partial_pcs, \
                                    ...
                                    "mani_point_1": mani_point_1, "mani_point_2": mani_point_2, "data type": data_type}
                        ...
                        data_type = "reverse"
                        pc_goal = pc_on_trajectory[0]
                        full_pc_goal = full_pc_on_trajectory[0]
                        final_trans_1 = curr_trans_on_trajectory_1[0]
                        final_trans_2 = curr_trans_on_trajectory_2[0]
                        # print("***Final x, y, z: ", final_pose["pose"]["p"]["x"], final_pose["pose"]["p"]["y"], final_pose["pose"]["p"]["z"] ) 
