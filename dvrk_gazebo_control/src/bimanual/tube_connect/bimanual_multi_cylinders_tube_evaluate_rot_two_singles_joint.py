...
def get_tube_connect_goal_pc(object_id):
    ...
    goal_pc = trimesh.sample.sample_surface_even(goal_mesh, count=2048)[0]    
...
if __name__ == "__main__":
    ...
    # soft_pose_1.p = gymapi.Vec3(0.1, -0.31, r/2.0)
    # soft_pose_1.p = gymapi.Vec3(0.18, -0.40, r/2.0)
    ...
    model.load_state_dict(torch.load(os.path.join(weight_path, "epoch 176"))) 

    pcd_goals = []
    goal_pc_tensors = []
    for id in [3]:
        goal_pc_numpy = get_tube_connect_goal_pc(object_id=id)
        goal_pc_numpy = down_sampling(np.array([p for p in goal_pc_numpy if p[2]>=0.01]))
