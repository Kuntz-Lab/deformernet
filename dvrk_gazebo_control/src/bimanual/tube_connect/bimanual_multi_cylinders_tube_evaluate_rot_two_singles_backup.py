...
def get_tube_connect_goal_pc(filename="curve.csv"):
    ...
    import csv
    # csv file name
    filename = "/home/baothach/Downloads/" + filename
...
if __name__ == "__main__":
    ...
    # soft_pose_1.p = gymapi.Vec3(0.06, -0.35, r/2.0)
    ...
    cam_targets.append(gymapi.Vec3(0.0, 0.40-0.86, 0.01))
    ...
    gym.set_camera_location(cam_handles_2[i], env_2, cam_positions[0], cam_targets[0])
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            rospy.loginfo("**Current state: " + state) 
            # _, current_pc_numpy = get_point_clouds(vis=False)
            # current_pc_numpy = down_sampling(current_pc_numpy)
            # current_pc_numpy = [p for p in get_partial_point_cloud(i) if p[0] <= 0.09]
            # current_pc_numpy = down_sampling(np.array(current_pc_numpy))
            current_pc_numpy = down_sampling(get_partial_point_cloud(object_id=1))
            ...
            modified_pc = np.vstack([current_pc_numpy.transpose(1,0), mp_channel_2])
            ...
            with torch.no_grad():
                pos_1, rot_mat_1 = model(current_pc_tensor, goal_pc_tensor)
                ...
                pos_1*=0.001
                pos_1[0][2] = 0.0
                desired_pos_1 = (pos_1 + init_pose_2[:3,3]).flatten()#???
                ...
                desired_rot_1 = rot_mat_1 @ init_pose_2[:3,:3]  #???
                # desired_rot_2 = rot_mat_2 @ init_pose_2[:3,:3]


                # desired_pos_2 = desired_pos_1
                # # pos_1[0][0] *= -1
                ...
                tvc_behavior_2 = TaskVelocityControl2([*desired_pos_1, desired_rot_1], robot_2, sim_params.dt, 3, vel_limits=vel_limits, use_euler_target=False, \
                                                    pos_threshold = 1e-3, ori_threshold=2e-2)
