...
if __name__ == "__main__":
    ...
    soft_pose_2.p = gymapi.Vec3(0.02, -0.8*two_robot_offset/3.5, r/2.0)
    # soft_pose_2.r = gymapi.Quat(0.7071068, 0, 0, 0.7071068)
    rot_angle = 0 #np.random.uniform(low = -np.pi/4.5, high = np.pi/4.5)
    ...
    soft_pose_1.p = gymapi.Vec3(-0.02, -2.3*two_robot_offset/3.5, r/2.0)
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            delta_x, delta_y, delta_z = -0.00,0.05,0.08
            ...
            cartesian_pose_1.position.x = current_pose_1["pose"]["p"]["x"] + delta_x 
            cartesian_pose_1.position.y = current_pose_1["pose"]["p"]["y"] + two_robot_offset + delta_y
            cartesian_pose_1.position.z = current_pose_1["pose"]["p"]["z"] - ROBOT_Z_OFFSET + delta_z 

            cartesian_pose_2 = deepcopy(cartesian_pose_1)
            cartesian_pose_2.position.x = -(current_pose_1["pose"]["p"]["x"] + delta_x)
            cartesian_pose_2.position.y = -(current_pose_1["pose"]["p"]["y"] + delta_y +0.05)
