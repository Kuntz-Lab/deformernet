...
if __name__ == "__main__":
    ...
    pos_1 = np.random.uniform(low = -0.03, high = 0.03)
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            if random_movement:
                max_x, max_y, max_z = 0.1, 0.03, 0.1
                delta_x_1 = np.random.uniform(low = -max_x, high = max_x)
                delta_y_1 = np.random.uniform(low = -max_y, high = max_y)
                delta_z_1 = np.random.uniform(low = 0.03, high = max_z)     

                delta_x_2 = np.random.uniform(low = -max_x, high = max_x)
                delta_y_2 = np.random.uniform(low = -max_y, high = max_y)
                delta_z_2 = np.random.uniform(low = 0.03, high = max_z)                  

                cartesian_pose_1.position.x = delta_x_1                 
                cartesian_pose_1.position.y = -two_robot_offset/2 + delta_y_1 + two_robot_offset
                cartesian_pose_1.position.z = current_pose_1["pose"]["p"]["z"] - ROBOT_Z_OFFSET + delta_z_1 
                
                cartesian_pose_2 = deepcopy(cartesian_pose_1)
                cartesian_pose_2.position.x = -delta_x_2               
                cartesian_pose_2.position.y = -(-two_robot_offset/2 + delta_y_2) 
                cartesian_pose_2.position.z = current_pose_2["pose"]["p"]["z"] - ROBOT_Z_OFFSET + delta_z_2    

            else:
                if preset_deltas is None:
                    gripper_offset = 0.05
                    ...
                    # delta_x, delta_y, delta_z = -0.1,0.05,0.1

                    cartesian_pose_1.position.x = delta_x                 
                    cartesian_pose_1.position.y = -two_robot_offset/2 - gripper_offset/2 + two_robot_offset
                    cartesian_pose_1.position.z = current_pose_1["pose"]["p"]["z"] - ROBOT_Z_OFFSET + delta_z 
                else:                
                    delta_x, _, delta_z = preset_deltas[0]
                    error = preset_deltas[1]
                    offsets = [error[0], error[1]-0.02 if error[1]>=0.02 else 0, error[2]]
                    rospy.logwarn(f"offsets: {offsets}")
                    
                    cartesian_pose_1.position.x = delta_x + offsets[0] 
                    cartesian_pose_1.position.y = -two_robot_offset/2 - gripper_offset/2 + two_robot_offset + offsets[1]
                    cartesian_pose_1.position.z = current_pose_1["pose"]["p"]["z"] - ROBOT_Z_OFFSET + delta_z + offsets[2]                

                cartesian_pose_2 = deepcopy(cartesian_pose_1)
                cartesian_pose_2.position.x = -delta_x    #-(current_pose_1["pose"]["p"]["x"] + delta_x)
                # cartesian_pose_2.position.y = -(current_pose_1["pose"]["p"]["y"] + delta_y +0.06)
                cartesian_pose_2.position.y = -(-two_robot_offset/2 + gripper_offset/2) 
                cartesian_pose_2.position.z = current_pose_1["pose"]["p"]["z"] - ROBOT_Z_OFFSET + delta_z      
        ...
        if state == "move to goal":
            ...
            if (not(20 in contacts or 21 in contacts) or not(9 in contacts or 10 in contacts)):  # lose contact w either robot 2 or robot 1    
                ...
                if frame_count % 150 == 0:
                    ...
                    # pcd = open3d.geometry.PointCloud()
                    # pcd.points = open3d.utility.Vector3dVector(down_sampling(get_partial_point_cloud(i), num_point=512))
                    # open3d.visualization.draw_geometries([pcd])           
                ...
                if traj_index_1 == len(plan_traj_1) and traj_index_2 == len(plan_traj_2):
                    ...
                    if not random_movement:
                        ...
                        if preset_deltas is None and (abs(error[0]) >= 0.003 or abs(error[2]) >= 0.003):
                            preset_deltas = [(delta_x, 0, delta_z), error]
                            # [delta_x+error[0], error[1]-0.02 if error[1]>=0.02 else 0, delta_z + error[2]]
