...
def transform_pc(pc):
    # Transform pc to robot 2 frame
    pc = np.array([[p[0]-soft_pose_1.p.x, p[1], p[2]] for p in pc])
    return pc

def flip_pc(pc):
    # flip wrt to y axis
    pc = np.array([[-p[0], p[1], p[2]] for p in pc])
    # pc = np.array([[soft_pose_1.p.x-p[0], p[1], p[2]] for p in pc])
    return pc

def get_tube_connect_goal_pc(object_id, return_backbone=False):
    ...
    if return_backbone:
        ys_shifted = ys + soft_pose_2.p.y-0.10
        goal_backbone = np.column_stack((xs, ys_shifted))
    ...
    T = trimesh.transformations.translation_matrix([-0.00,soft_pose_2.p.y-0.10,0.01])
    # if object_id == 1:
    #     T = trimesh.transformations.translation_matrix([-0.01,soft_pose_2.p.y+0.03,0.01])
    ...
    goal_pc = trimesh.sample.sample_surface_even(goal_mesh, count=1680)[0]    
...
if __name__ == "__main__":
    ...
    # soft_pose_1.p = gymapi.Vec3(0.17, -0.40, r/2.0)# 2_curves_data_2.csv   
    # soft_pose_1.p = gymapi.Vec3(0.17, -0.27, r/2.0)  # 2_curves_data_3.csv
    # # soft_pose_1.p = gymapi.Vec3(0.1, -0.31, r/2.0) #one top one bottom
    # soft_pose_1.p = gymapi.Vec3(0.13, -0.35, r/2.0)
    soft_pose_1.p = gymapi.Vec3(0.21, -0.46, r/2.0)
    ...
    pose.p = gymapi.Vec3(soft_pose_1.p.x, 0, ROBOT_Z_OFFSET) 
    ...
    for i in range(num_envs):
        ...
        envs_2.append(env_2)        
        soft_actor_2 = gym.create_actor(env_2, soft_asset, soft_pose_2, "soft_2", i+1, 0)
    ...
    cam_position = gymapi.Vec3(0.17+0.1, -0.62, 0.2)
    ...
    # cam_position=gymapi.Vec3(0.20, pose.p.y/2, 0.2)
    # cam_target=gymapi.Vec3(0.0, pose.p.y/2, 0.01)    
...
# np.array([[-(p[0]-pose.p.x), -(p[1]-pose.p.y), p[2]] for p in pc])
    ...
    # weight_path = "/home/baothach/shape_servo_data/rotation_extension/multi_cylinders_1000Pa/weights/run3_single_thin_on_ground/"    
    # model.load_state_dict(torch.load(os.path.join(weight_path, "epoch 290"))) 
    weight_path = "/home/baothach/shape_servo_data/rotation_extension/multi_cylinders_1000Pa/weights/run3_single_thin_on_ground_full_pc_goal/"
    model.load_state_dict(torch.load(os.path.join(weight_path, "epoch 300"))) 
    pcd_goals = []
    goal_pc_tensors = []
    goal_backbones = []
    ...
    frechets_1 = []
    frechets_2 = []
    end_pos_1 = [] # position of the cylinders' end
    end_pos_2 = []

    for id in [1,2]:
        goal_pc_numpy = get_tube_connect_goal_pc(object_id=2)
        ...
        if id == 1:
            ...
            pcd_goal.points = open3d.utility.Vector3dVector(flip_pc(goal_pc_numpy))
            goal_pc_tensor = torch.from_numpy(flip_pc(goal_pc_numpy)).permute(1,0).unsqueeze(0).float().to(device)
            ...
            # goal_pc_numpy = flip_pc(goal_pc_numpy)
            # pcd_goal = open3d.geometry.PointCloud()
            # pcd_goal.points = open3d.utility.Vector3dVector(goal_pc_numpy)
            goal_pc_tensor = torch.from_numpy(goal_pc_numpy).permute(1,0).unsqueeze(0).float().to(device)
        ...
        goal_backbones.append(get_tube_connect_goal_pc(object_id=id, return_backbone=True))


    # print("goal_backbone_2.shape: ", goal_backbone_2.shape)
    metric_count = 0
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            if frame_count == 5:
                ...
                if first_time:                    
                    ...
                    # # min_z = min(pc_2[:,2])
                    # # print("min_z:", min_z)
                    # # backbone = np.array([p for p in pc_2 if p[2] <= 0.001 and abs(p[0]) <= 0.001])
                    
                    

                    # zs = pc_2[:,2]
                    # z_idxs = np.where(zs <= 0.001)[0]
                    # xs = pc_2[:,0]
                    ...
                    # backbone_idxs_2 = backbone_idxs[sampled_idx]                  

                    # zs = pc_1[:,2]
                    # z_idxs = np.where(zs <= 0.001)[0]
                    # xs = pc_1[:,0]
                    # x_idxs = np.where(abs(xs-pose.p.x) <= 0.002)[0]
                    # backbone_idxs = np.array(list(set(z_idxs) & set(x_idxs)))
                    # sampled_idx = np.random.randint(low = 0, high=backbone_idxs.shape, size= goal_backbones[0].shape[0])               
                    # backbone_idxs_1 = backbone_idxs[sampled_idx]     

                    # compound_bb_idxs = [backbone_idxs_1, backbone_idxs_2]             


                    # # backbone_1 = pc_1[backbone_idxs_1]
                    # # backbone_pcd_1 = open3d.geometry.PointCloud()
                    # # backbone_pcd_1.points = open3d.utility.Vector3dVector(backbone_1)
                    # # backbone_pcd_1.paint_uniform_color([0,1,0]) 
                    # # backbone_2 = pc_2[backbone_idxs_2]
                    ...
                    # # open3d.visualization.draw_geometries([pcd, backbone_pcd_1])                     


                    # zs = pc_2[:,2]
                    # z_idxs = np.where(abs(zs-0.01) <= 0.001)[0]
                    # xs = pc_2[:,0]
                    # x_idxs = np.where(abs(xs) <= 0.002)[0]
                    # backbone_idxs_2 = np.array(list(set(z_idxs) & set(x_idxs)))
                    # center_2_idx = max(range(pc_2[backbone_idxs_2].shape[0]), key=lambda i: pc_2[backbone_idxs_2][i][1])
                    # # certer_2 = pc_2[backbone_idxs_2][center_2_idx]
                    # # # certer_2 = max(pc_2[backbone_idxs], key=lambda x:x[1])                      
                    # # center_pcd_2 = open3d.geometry.TriangleMesh.create_sphere(radius=0.01)
                    # # center_pcd_2.paint_uniform_color([0,0,1])
                    # # center_pcd_2.translate(tuple(certer_2))
                    # # # open3d.visualization.draw_geometries([pcd, center_pcd_2])  

                    # zs = pc_1[:,2]
                    # z_idxs = np.where(abs(zs-0.01) <= 0.001)[0]
                    # xs = pc_1[:,0]
                    # x_idxs = np.where(abs(xs-pose.p.x) <= 0.002)[0]
                    # backbone_idxs_1 = np.array(list(set(z_idxs) & set(x_idxs)))
                    # center_1_idx = min(range(pc_1[backbone_idxs_1].shape[0]), key=lambda i: pc_1[backbone_idxs_1][i][1])
                    # # certer_1 = pc_1[backbone_idxs_1][center_1_idx]
                    # # # certer_1 = max(pc_1[backbone_idxs], key=lambda x:x[1])                      
                    # # center_pcd_1 = open3d.geometry.TriangleMesh.create_sphere(radius=0.01)
                    # # center_pcd_1.paint_uniform_color([0,0,1])
                    # # center_pcd_1.translate(tuple(certer_1))
                    # # open3d.visualization.draw_geometries([pcd, center_pcd_1, center_pcd_2])  
        ...
        #         pc_1, pc_2 = get_point_clouds()

        #         for t, pc in enumerate([pc_1, pc_2]):
        #             backbone = pc[compound_bb_idxs[t]][:,:2]
        #             xs = backbone[:,0]
        #             ys = backbone[:,1]
        ...
        #             xs = xs[sorted_idx]
        #             ys = ys[sorted_idx]
        #             backbone = np.column_stack((xs, ys))

        #             # quantify the difference between the two curves using
        #             # Discrete Frechet distance
        #             df = similaritymeasures.frechet_dist(goal_backbones[t], backbone)    
        #             print(f"Frechet distance {t+1}: {df}")  
        ...
        #                 end_pos_1.append(pc_1[backbone_idxs_1][center_1_idx])
        #             else:
        #                 plt.plot(xs, ys, 'g')
        #                 plt.plot(goal_backbones[t][:,0], goal_backbones[t][:,1], 'bo')
        #                 backbone_data_2.append([xs, ys])
        #                 frechets_2.append(df)
        #                 end_pos_2.append(pc_2[backbone_idxs_2][center_2_idx])

        #             plt.savefig("/home/baothach/dvrk_shape_servo/src/dvrk_env/dvrk_gazebo_control/src/bimanual/tube_connect/\
        #                             test_frechet.png")
        ...
        if state == "generate preshape":                   
            ...
            cartesian_goal = deepcopy(preshape_response.palm_goal_pose_world[0].pose)        
            target_pose = [cartesian_goal.position.x-pose.p.x, -cartesian_goal.position.y, cartesian_goal.position.z-ROBOT_Z_OFFSET,
                            0, 0.707107, 0.707107, 0]
        ...
        if state == "get shape servo plan":
            ...
            current_pc_numpy_1 = transform_pc(down_sampling(get_partial_point_cloud(object_id=1)))  # shift current pc to origin
            ...
            mani_point_1 = np.array([mani_point_1[0]-soft_pose_1.p.x, mani_point_1[1], mani_point_1[2]])
            ...
            # pcs = np.concatenate((current_pc_numpy_1, current_pc_numpy_2), axis=0)
            # pcd = open3d.geometry.PointCloud()
            ...
            with torch.no_grad():
                ...
                # (nhin doi dien voi robot): pitch, yaw, roll
                # pos_1, rot_mat_1 = deepcopy(pos_2), deepcopy(rot_mat_2)
                ...
                # euler_1 = np.array(list(transformations.euler_from_matrix(temp_1)))
                # # euler_1[1] *= -1 
                # euler_1[2] *= -1
                # rot_mat_1 = transformations.euler_matrix(*euler_1)[:3,:3]
        ...
        if state == "move to goal":
            ...
            if action_1 is None or action_2 is None or  gym.get_sim_time(sim) - closed_loop_start_time >= 3:   
                ...
                # backbone = pc_2[backbone_idxs]
                # backbone_pcd = open3d.geometry.PointCloud()
                # backbone_pcd.points = open3d.utility.Vector3dVector(backbone)
                # backbone_pcd.paint_uniform_color([0,1,0])
                # open3d.visualization.draw_geometries([pcd, backbone_pcd])    
    ...
    data = {"goal_bb_1": [goal_backbones[0][:,0], goal_backbones[0][:,1]], \
            "goal_bb_2": [goal_backbones[1][:,0], goal_backbones[1][:,1]], \
            "backbone_data_1": backbone_data_1, "backbone_data_2": backbone_data_2, \
            "frechets_1": frechets_1, "frechets_2": frechets_2,
            "end_pos_1": end_pos_1, "end_pos_2": end_pos_2}

    path = "/home/baothach/dvrk_shape_servo/src/dvrk_env/dvrk_gazebo_control/src/bimanual/tube_connect/vis"
    with open(os.path.join(path, "vis_data_4.pickle"), 'wb') as handle:
