...
if __name__ == "__main__":
    ...
    cam_position=gymapi.Vec3(0.2, -two_robot_offset/2, 0.2)
...
# np.array([[-(p[0]-pose.p.x), -(p[1]-pose.p.y), p[2]] for p in pc])
    ...
    result_save_path = "/home/baothach/shape_servo_data/tube_connect/cylinder_rot_attached/results"
    os.makedirs(result_save_path, exist_ok=True)
    terminate_count = 0
    sample_count = 0
    frame_count = 0
    group_count = 0
    data_point_count = len(os.listdir(result_save_path))
    ...
    weight_path = "/home/baothach/shape_servo_data/rotation_extension/multi_cylinder_1kPa/weights/run3_single_thin_on_ground_full_pc_goal/"
    model.load_state_dict(torch.load(os.path.join(weight_path, "epoch 300")))     
    pcd_goals = []
    goal_pc_tensors = []
    goal_backbones = []
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state not in ["home", "move to preshape", "grasp object"]:
        ...
        if state == "get shape servo plan":
            ...
            # pcd_2 = open3d.geometry.PointCloud()
            # pcd_2.points = open3d.utility.Vector3dVector(current_pc_numpy_2)
            ...
            # open3d.visualization.draw_geometries([pcd, pcd_2, mani_point_1_sphere, mani_point_2_sphere, pcd_goal]) 
            ...
            with torch.no_grad():
                ...
                euler_2 = np.array(list(transformations.euler_from_matrix(temp_2)))
                euler_2[0] = 0 
                rot_mat_2 = transformations.euler_matrix(*euler_2)[:3,:3]

                desired_rot_1 = rot_mat_1 @ init_pose_1[:3,:3]
                desired_rot_2 = rot_mat_2 @ init_pose_2[:3,:3]


                # remove last three rows in curve 2 and first 3 rows in curve 1
        ...
        if state == "move to goal":
            ...
            if timeit.default_timer() - shapesrv_start_time >= max_shapesrv_time:
                ...
                if action_1 is None or action_2 is None or  gym.get_sim_time(sim) - closed_loop_start_time >= 3:   
                    ...
                    current_pc_numpy_1 = get_partial_point_cloud(object_id=1) # shift current pc to origin
                    current_pc_numpy_2 = get_partial_point_cloud(object_id=2)           
                ...
                # Terminal conditions
                threshold = 0.006
                if all(abs(pos_1.squeeze()) <= threshold) and all(abs(pos_2.squeeze()) <= threshold):                   
                    state = "reset"
    ...
    with open(os.path.join(result_save_path, "sample " + str(data_point_count) + ".pickle"), 'wb') as handle:
