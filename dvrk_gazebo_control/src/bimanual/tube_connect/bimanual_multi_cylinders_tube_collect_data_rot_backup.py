...
if __name__ == "__main__":
    ...
    max_sample_count = 4 if random_movement else 5
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            if not maintain_alignment:
                ...
                # max_y_1 = two_robot_offset/2 - (0.4/(2*2) - abs(current_pose_1['y'] - soft_pose_1.p.y)) - 0.01
                # max_y_2 = two_robot_offset/2 - (0.4/(2*2) - abs(current_pose_2['y'] - soft_pose_2.p.y)) - 0.01
                # print("max_y_1, max_y_2:", max_y_1, max_y_2)

                delta_x_1 = np.random.uniform(low = -max_x, high = max_x)
                delta_y_1 = two_robot_offset/3.5 + abs(abs(current_pose_1["y"]) - 2.15*two_robot_offset/3.5) #np.random.uniform(low = two_robot_offset/3.5 + init_pose_2[1,3] - 1.35*two_robot_offset/3.5, high = init_pose_2[1,3]) 
                ...
                delta_y_2 = two_robot_offset/3.5 + abs(abs(current_pose_2["y"]) - 1.35*two_robot_offset/3.5) #np.random.uniform(low = two_robot_offset/3.5 + init_pose_2[1,3] - 1.35*two_robot_offset/3.5, high = init_pose_2[1,3])
        ...
        if state == "move to goal":
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if frame_count % 150 == 0:
                    ...
                    curr_trans_on_trajectory_2.append(get_pykdl_client(robot_1.get_arm_joint_positions())[1])
                ...
                if not(action_1 is None or action_2 is None) and gym.get_sim_time(sim) - closed_loop_start_time <= 3:   
                    ...
                    if random_movement or maintain_alignment:                    
                        ...
                        data_type = "no maintain alignment"
