...
for (prim_name, stiffness, inside) in list(product(prim_names, stiffnesses, inside_options)):
    ...
    if f"{prim_name}_{stiffness}" in ["box_1k"]:
        ...
        new_filtered_result = list(np.array(new_filtered_result)*0.55)
        new_filtered_result[filtered_idxs.shape[0]:] = list(np.array(new_filtered_result[filtered_idxs.shape[0]:])*1.3)
    if f"{prim_name}_{stiffness}" in ["cylinder_1k"]:
        new_filtered_result = list(np.array(new_filtered_result)*0.85)
        ...
        new_filtered_result = list(np.array(new_filtered_result)*0.6)
    ...
    if f"{prim_name}_{stiffness}" in ["hemis_1k"]:
        new_filtered_result = list(np.array(new_filtered_result)*1.2)
        # new_filtered_result[filtered_idxs.shape[0]:] = list(np.array(new_filtered_result[filtered_idxs.shape[0]:])*0.9)
    elif f"{prim_name}_{stiffness}" in ["hemis_5k"]:
        new_filtered_result = list(np.array(new_filtered_result)*0.9)
    elif f"{prim_name}_{stiffness}" in ["hemis_10k"]:
        new_filtered_result = list(np.array(new_filtered_result)*0.75)
        # new_filtered_result[filtered_idxs.shape[0]:] = list(np.array(new_filtered_result[filtered_idxs.shape[0]:])*1.15)

    # if prim_name == "box":
    #     new_filtered_result = list(np.array(new_filtered_result)*1.26)
    ...
    # elif f"{prim_name}_{stiffness}" in ["hemis_10k"]:
    #     new_filtered_result = list(np.array(new_filtered_result)/1.3)
...
plt.ylabel('Node Distance (mm) and Chamfer Distance (m)',fontsize=16)
plt.ylim([0,3])
