...
from utils.miscellaneous_utils import get_extents_object
import pickle
...
if __name__ == "__main__":
    ...
    # main_path = "/home/baothach/shape_servo_data/rotation_extension/multi_box_5kPa/evaluate"
    ...
    object_meshes_path = f"/home/baothach/sim_data/Custom/Custom_mesh/physical_dvrk/multi_{args.obj_type}Pa"    
    ...
    if unseen_obj_name is not None:
        ...
        h = round(np.array(extents[1][1])-np.array(extents[0][1]), 2)
        thickness = round(np.array(extents[1][2])-np.array(extents[0][2]), 2)
        w = round(np.array(extents[1][0])-np.array(extents[0][0]), 2)
    ...
    goal_recording_path = f"/home/baothach/shape_servo_data/rotation_extension/single_physical_dvrk/goal_data/{args.obj_type}" 
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            # delta_alpha = np.random.uniform(low = np.pi/6, high = np.pi/3)
            # delta_beta = np.random.uniform(low = np.pi/6, high = np.pi/3) 
            # delta_gamma = np.random.uniform(low = np.pi/3, high = np.pi/2)
            # delta_alpha *= np.random.choice([-1,1])
            # delta_beta *= np.random.choice([-1,1])
            # delta_gamma *= np.random.choice([-1,1])
