...
def get_results(prim_name, stiffness, range_data):
    object_category = f"{prim_name}_{stiffness}"
    main_path = f"/home/baothach/shape_servo_data/rotation_extension/bimanual_physical_dvrk/multi_{object_category}Pa/evaluate/chamfer_results/{args.frame_name}_frame"   

    chamfer_data = []
    chamfer_data_avg = []
    for i in range_data:
        file_name= os.path.join(main_path, f"{prim_name}_{str(i)}.pickle")
        # print(file_name)
...
parser.add_argument('--frame_name', default="object", type=str, help="name of point cloud frame", choices=["object", "world"])
...
for (prim_name, stiffness) in list(product(prim_names, stiffnesses)):
    ...
    res, res_avg = get_results(prim_name, stiffness, range_data=range_data)
