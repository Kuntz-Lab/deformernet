...
from utils.point_cloud_utils import down_sampling, world_to_object_frame, object_to_world_frame, transform_point_cloud
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load(os.path.join(weight_path, f"epoch {150}")))    # 200
    model.eval()
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            if object_frame:
                transformation_matrix = world_to_object_frame(current_pc_numpy)
                current_pc_numpy = transform_point_cloud(current_pc_numpy, transformation_matrix)
                
                transformed_goal_pc_numpy = transform_point_cloud(goal_pc_numpy - shift, transformation_matrix)
                ...
                mani_point_1 = transform_point_cloud(mani_point_1.reshape(1, -1), transformation_matrix).flatten()
                mani_point_2 = transform_point_cloud(mani_point_2.reshape(1, -1), transformation_matrix).flatten()
            else:
