...
def get_partial_pointclouds_multi_cameras():
    partial_pcs = []    # list of 8 point clouds from 8 different camera views
    for cam_handle in cam_handles:
        camera_args[3] = cam_handle
        partial_pc = get_partial_pointcloud_vectorized(*camera_args) + shift        
        partial_pcs.append(partial_pc) # shape (num_pts,3)
...
if __name__ == "__main__":
    ...
    targets = np.random.uniform(low=[-0.03, -0.03, 0.01], high=[0.03, 0.03, 0.04], size=(num_cams,3))
    targets[:,1] += soft_pose.p.y
    pc_cam_targets = [gymapi.Vec3(targets[i][0], targets[i][1], targets[i][2]) for i in range(num_cams)]

    delta = 0.3
    # cam_pos_zs = np.random.uniform(low=0.3, high=delta, size=num_cams)
    cam_pos_zs = np.random.uniform(low=0.2, high=0.4, size=num_cams)
    cam_pos_xs = np.random.uniform(low=-delta, high=delta, size=num_cams) 
    cam_pos_ys = np.random.uniform(low=-delta, high=delta, size=num_cams) + soft_pose.p.y


    
    pc_cam_positions = []        
    for c in range(num_cams):
        pc_cam_positions.append(gymapi.Vec3(cam_pos_xs[c],cam_pos_ys[c],cam_pos_zs[c]))

    for k in range(num_cams):
        cam_handle = gym.create_camera_sensor(env_obj, cam_props)
        cam_handles.append(deepcopy(cam_handle))
        gym.set_camera_location(cam_handle, env_obj, pc_cam_positions[k], pc_cam_targets[k])
    ...
    data_recording_path = f"/home/baothach/shape_servo_data/rotation_extension/single_physical_dvrk/multi_{object_category}Pa_multi_cameras/data_2"
    # mp_data_recording_path = f"/home/baothach/shape_servo_data/manipulation_points/single_physical_dvrk/multi_{object_category}Pa/mp_data"
    ...
    max_sample_count = 3
    max_data_point_count = 500000     #5000
    ...
    camera_args = [gym, sim, envs_obj[0], None, cam_props, 
                    segmentationId_dict, "deformable", None, 0.004, False, "cpu"]   # 0.002    
    visualization = False
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            if frame_count == 10:
                ...
                # camera_args[3] = cam_handles[0]
                # pc = get_partial_pointcloud_vectorized(*camera_args)
                # pcd_ize(pc, vis=True)
        ...
        if state == "generate preshape":                   
            ...
            pc_init = get_partial_pointclouds_multi_cameras()
        ...
        if state == "get shape servo plan":
            ...
            max_x = max_y = max_z = h * 3/4  #h * 0.7 * 0.8 
            # max_z = h / 2.0

            delta_alpha = np.random.uniform(low = -np.pi/8, high = np.pi/8)
            delta_beta = np.random.uniform(low = -np.pi/8, high = np.pi/8) 
            delta_gamma = np.random.uniform(low = -np.pi/8, high = np.pi/8)
        ...
        if state == "move to goal":
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if frame_count % 15 == 0: #15
                    ...
                    pc_on_trajectory.append(get_partial_pointclouds_multi_cameras())
                ...
                if action is not None and gym.get_sim_time(sim) - closed_loop_start_time <= 1.5: 
                    ...
                    pc_goal = get_partial_pointclouds_multi_cameras()
                    ...
                    for j, curr_trans in enumerate(curr_trans_on_trajectory):                        
                        p, R = tvc_behavior.get_transform(curr_trans, final_trans, get_twist=False)
                        mani_point = np.array([-curr_trans[0,3], -curr_trans[1,3], curr_trans[2,3] + ROBOT_Z_OFFSET]) + shift
                        ...
                        # pcds_goal = []
                        # offset = 0.15
                        # for j, (pc, pc_goal) in enumerate(zip(pc_on_trajectory[j], pc_goal)):
                        ...
                        data = {"partial pcs": partial_pcs, "pos": p, "rot": R, \
                    ...
                    #         mani_point = np.array(list(mp_mani_point["pose"][0])) + shift

                    #         ### Record data for training manipulation point predictors.
                    #         data = {"full pcs": full_pcs, "partial pcs": partial_pcs, "mani_point": mani_point, "obj_name": args.obj_name}
