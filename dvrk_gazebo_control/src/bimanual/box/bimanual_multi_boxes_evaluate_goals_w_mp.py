...
if __name__ == "__main__":
    ...
    chamfer_recording_path =  "/home/baothach/shape_servo_data/bimanual/multi_boxes_1000Pa/evaluate/chamfer_results_w_pred_mp"
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "generate preshape":                   
            ...
            best_mp_1 = current_pc_numpy[np.argmax(success_probs_1)]
            best_mp_2 = current_pc_numpy[np.argmax(success_probs_2)]

            target_pose = [-best_mp_2[0], -best_mp_2[1], best_mp_2[2]-ROBOT_Z_OFFSET,
                            0, 0.707107, 0.707107, 0]
            mtp_behavior_2 = MoveToPose(target_pose, robot_2, sim_params.dt, 1)
                
            target_pose = [best_mp_1[0], best_mp_1[1] + two_robot_offset, best_mp_1[2]-ROBOT_Z_OFFSET,
                            0, 0.707107, 0.707107, 0]
        ...
        if state == "move to goal":
            ...
            if main_ins_pos_1 <= 0.042 or main_ins_pos_2 <= 0.042 or not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                rospy.logerr("Lost contact with robot or Exceed joint constraint")
                state = "reset" 
