...
if __name__ == "__main__":
    ...
    # pose_2.p = gymapi.Vec3(0.0, -1.0, ROBOT_Z_OFFSET)
    ...
    asset_root = "/home/baothach/sim_data/Custom/Custom_urdf/retraction_cutting"
    soft_asset_file = "box_1.urdf"    
    ...
    data_recording_path = "/home/baothach/shape_servo_data/rotation_extension/multi_box_10kPa/data"
    # elif args.data_category == "MP":
    #     data_recording_path = "/home/baothach/shape_servo_data/manipulation_points/multi_boxes_10kPa/mp_data" 

    mp_data_recording_path = "/home/baothach/shape_servo_data/manipulation_points/multi_box_10kPa/mp_data"
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            if frame_count == 10:
                ...
                # print("base frame:", gym.get_actor_rigid_body_states(env, kuka_handles_2[i], gymapi.STATE_POS)[0]["pose"])
                # print("eef frame:", gym.get_actor_rigid_body_states(env, kuka_handles_2[i], gymapi.STATE_POS)[-3]["pose"])
                
                # frames = []
                # for k in range(1,9):
                #     frames.append(gym.get_actor_rigid_body_states(env, kuka_handles_2[i], gymapi.STATE_POS)[k]["pose"])
                # print("frames:", ', '.join(str(f) for f in frames))
                
                # joint_angles = gym.get_actor_dof_states(envs[i], kuka_handles_2[i], gymapi.STATE_POS)['pos'][:8]
                # print("joint angles:", ', '.join(str(num) for num in joint_angles))
        ...
        if state == "generate preshape":                   
            ...
            target_pose = [-cartesian_goal.position.x+0.1, -cartesian_goal.position.y, cartesian_goal.position.z-ROBOT_Z_OFFSET+0.1,
                            0, 0.707107, 0.707107, 0]
            # target_pose = [cartesian_goal.position.x+0.15, cartesian_goal.position.y+1.0, cartesian_goal.position.z-ROBOT_Z_OFFSET+0.12,
            #                 0, 0.707107, 0.707107, 0]
        ...
        if state == "get shape servo plan":
            ...
            delta_alpha = np.pi/300000
            delta_beta =  np.pi/300000
            delta_gamma = np.pi/300000

            delta_x = max_x
            delta_y = max_y
            delta_z = max_z 

            # delta_alpha, delta_beta, delta_gamma = 0, -np.pi/3, 0
        ...
        if state == "move to goal":
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if action is not None and gym.get_sim_time(sim) - closed_loop_start_time <= 8: 
                    ...
                    # gym.set_actor_dof_velocity_targets(robot.env_handle, robot.robot_handle, [0,0,0,0,0,0,0,0]) 
                else:   
                    ...
                    data = {"tet": (tet_indices, tet_stress), "tri": (tri_indices, tri_parents, tri_normals), "particles": get_point_cloud()}
                    save_path = "/home/baothach/shape_servo_data/retraction_cutting/test"
                    with open(os.path.join(save_path, "test_0.pickle"), 'wb') as handle:
