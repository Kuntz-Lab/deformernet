...
if __name__ == "__main__":
    ...
    if sim_type is gymapi.SIM_FLEX:
        ...
        sim_params.flex.num_outer_iterations = 10 #4 10
    ...
    asset_root = "/home/baothach/shape_servo_data/retraction_cutting/multi_box/evaluate/Custom_urdf_2"
    soft_asset_file = args.obj_name + ".urdf"   

    # Get primitive shape dictionary to know the dimension of the object   
    object_meshes_path = "/home/baothach/shape_servo_data/retraction_cutting/multi_box/evaluate/Custom_mesh_2"  
    ...
    cam_positions.append(gymapi.Vec3(0.2, -0.7, 0.2))
    cam_targets.append(gymapi.Vec3(0.0, -0.46, 0.01))
    ...
    max_data_collection_time = 2*60    # 2 mins
    max_grasping_time = 0.5*60    # 0.5 mins
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "grasp object":             
            ...
            if timeit.default_timer() - start_data_collection >= max_grasping_time:
                rospy.logerr("Grasping takes too long. Resetting ...")
                state = "reset" 
        ...
        if state == "get shape servo plan":
            ...
            # delta_x = np.random.uniform(low = max_x/2, high = max_x) * np.random.choice([-1,1]) 
            # delta_y = np.random.uniform(low = 0.0, high = max_y)
            # delta_z = np.random.uniform(low = 0.01, high = max_z)    
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if action is not None and gym.get_sim_time(sim) - closed_loop_start_time <= 8: 
                    ...
                    for j in range(len(pc_on_trajectory)):
                        mani_point = poses_on_trajectory[j]
                        
                        data = {"full_pc": full_pc_on_trajectory[j], "partial_pc": pc_on_trajectory[j], \
                                "tet": tetrahedra_on_trajectory[j], "tri": triangle_on_trajectory[j], \
