...
if __name__ == "__main__":
    ...
    if not args.headless:
        ...
        cam_pos = gymapi.Vec3(0.2, -0.7, 0.2)
        cam_target = gymapi.Vec3(0.0, -0.46, 0.01)        
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "home" :   
            ...
            if frame_count == 10:
                ...
                # start_time = timeit.default_timer()
                # get_partial_pointcloud_vectorized(gym, sim, envs_obj[0], cam_handles[0], cam_props, color=None, min_z=0.01, visualization=False, device="cuda")
                # print("GPU rendering time:", timeit.default_timer()-start_time)
                # # print(pc.shape)

                # start_time = timeit.default_timer()
                # get_partial_point_cloud(i)
                # print("Old rendering time:", timeit.default_timer()-start_time)
        ...
        if state == "move to goal":
            ...
            #     get_partial_pointcloud_vectorized(gym, sim, env, cam_handles[0], cam_props, color=None, min_z=0.005, visualization=True, device="cuda")
            ...
            if not(9 in contacts or 10 in contacts):  # lose contact w 1 robot
                ...
                if action is not None and gym.get_sim_time(sim) - closed_loop_start_time <= 8: 
                    ...
                    gym.render_all_camera_sensors(sim)
                    image_path = "/home/baothach/shape_servo_data/retraction_cutting/test/test.png"
