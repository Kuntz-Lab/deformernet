...
def get_point_cloud(vis=False):
    ...
    if vis:
        ...
        pcd.points = open3d.utility.Vector3dVector(np.array(down_sampling(point_cloud, num_point=256)))
        pcd.paint_uniform_color([1,0,0])
        
        tet_mesh = open3d.geometry.TetraMesh()
        tet_mesh = tet_mesh.create_from_point_cloud(pcd)[0]
        tet_mesh.paint_uniform_color([1,0,0])
...
if __name__ == "__main__":
    ...
    vis_cam_positions.append(gymapi.Vec3(0.15, 0.2-two_robot_offset, 0.2))   # 2 bis
    ...
    # constrain_plane = np.array([-1, 1, 0, 0.5])
