...
radius = 0.3
origin = radius/0.2 * 0.1 #* 4/3
mesh1 = trimesh.creation.icosphere(radius = radius)   # hemisphere
# mesh1 = trimesh.intersections.slice_mesh_plane(mesh=mesh1, plane_normal=[0,0,1], plane_origin=[0,0,0.15], cap=True)
mesh1 = trimesh.intersections.slice_mesh_plane(mesh=mesh1, plane_normal=[0,0,1], plane_origin=[0,0,origin], cap=True)

vs = np.array(mesh1.vertices)
# print(vs)
print("num faces: ", len(mesh1.faces))
vs_transformed = vs * np.array([1/1,1,1])
# print(vs_transformed)
mesh2 = trimesh.Trimesh(vertices=vs_transformed,
                       faces=mesh1.faces)
T = trimesh.transformations.translation_matrix([0.5, 0, 0])
mesh2 = trimesh.intersections.slice_mesh_plane(mesh=mesh2, plane_normal=[0,0,1], plane_origin=[0,0,origin], cap=True)
mesh2.apply_transform(T)

# mesh3 = mesh2.simplify_quadratic_decimation(face_count=400)
# print(mesh3)
# mesh3.apply_transform(T)

# z_s = vs[:,2]
# print(min(z_s), max(z_s), max(z_s)-min(z_s))

y_s = vs[:,1]
print(min(y_s), max(y_s), (max(y_s)-min(y_s))*2)
...
# mesh_dir = "/home/baothach/sim_data/Custom/Custom_mesh/multi_boxes_1000Pa_2"
