...
# mesh_wrap_tissue = trimesh.creation.cylinder(radius=0.015, height=0.1)    # tissue wrap cylinder_wrap_tissue.urdf
...
def create_box_mesh_datatset(save_mesh_dir, num_mesh=100, save_pickle=True, seed=0):
    np.random.seed(seed)
    primitive_dict = {'count':0}
    for i in range(num_mesh):
        print(f"object {i}")
...
# mesh_dir = "/home/baothach/sim_data/Custom/Custom_mesh/multi_boxes_10kPa"
...
def create_evaluation_new_hemis_mesh_datatset(save_mesh_dir, type, start_idx=0, num_mesh=10, inside=True, seed=0):
    np.random.seed(seed)
    random.seed(seed)
    ratios_print = []
    radius_print = []
    for i in range(start_idx, num_mesh):
        ...
        if inside:        
            ...
            radius = np.random.uniform(low = 0.2, high = 0.25)
            origin = radius/0.2 * 0.1 
            ratio = np.random.uniform(low = 2.5, high = 4)            

            ratios_print.append(ratio)
            radius_print.append(radius)
            ...
            # radius = np.random.uniform(low = 0.2, high = 0.3)
            radius = random.uniform(*random.choice([(0.1, 0.2), (0.3, 0.4)]))
            origin = radius/0.2 * 0.1          
    ...
    print("ratios:", np.array(ratios_print))
    print("radii:", np.array(radius_print))
    print("==========================")


mesh_dir = "/home/baothach/shape_servo_data/evaluation/meshes/hemis_10k/inside"
os.makedirs(mesh_dir,exist_ok=True)
create_evaluation_new_hemis_mesh_datatset(mesh_dir, type='10k', num_mesh=10, seed=2000) #1000


# mesh_dir = "/home/baothach/shape_servo_data/evaluation/meshes/box_10k/inside"
# os.makedirs(mesh_dir,exist_ok=True)
# create_evaluation_new_box_mesh_datatset(mesh_dir, type='10k')
