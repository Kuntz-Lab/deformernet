...
if __name__ == "__main__":
    ...
    # syrup_asset_file = "3m_high_tack_spray_adhesive.urdf"
    # syrup_asset_file = "advil_liqui_gels.urdf"

    syrup_pose = gymapi.Transform()
    syrup_pose.p = gymapi.Vec3(-0.1, 0.4, 0.0)
    ...
    for i, env in enumerate(envs):
        visual_cam_handles.append(gym.create_camera_sensor(env, visual_cam_props))
        gym.set_camera_location(visual_cam_handles[i], env, visual_cam_position, visual_cam_target)
    ...
    for i in range(num_envs): 
        ...
        dc_clients[i].cur_object_id = 0
        dc_clients[i].object_name = "custom_box" + str(i)
    ...
    while (not gym.query_viewer_has_closed(viewer)) and (not all_done):
        ...
        for i in range(num_envs):  
            ...
            if dc_clients[i].state == "get point cloud":
                ...
                if dc_clients[i].frame_count == 60:
                    rospy.loginfo("**Current state: " + dc_clients[i].state)
                    ...
                    depth_buffer = gym.get_camera_image(sim, envs[i], cam_handles[i], gymapi.IMAGE_DEPTH)
                    seg_buffer = gym.get_camera_image(sim, envs[i], cam_handles[i], gymapi.IMAGE_SEGMENTATION)

                    # Get the camera view matrix and invert it to transform points from camera to world
                    # space
                    vinv = np.linalg.inv(np.matrix(gym.get_camera_view_matrix(sim, envs[0], cam_handles[0])))

                    # Get the camera projection matrix and get the necessary scaling
                    # coefficients for deprojection
                    proj = gym.get_camera_proj_matrix(sim, envs[i], cam_handles[i])
                    fu = 2/proj[0, 0]
                    fv = 2/proj[1, 1]
            ...
            if dc_clients[i].state == "generate preshape":
                ...
                gym.write_camera_image_to_file(sim, envs[i], visual_cam_handles[i], gymapi.IMAGE_COLOR, dc_clients[i].get_rgb_image_save_path(lift=False)) # Need fix, choose the right camera
                gym.write_camera_image_to_file(sim, envs[i], visual_cam_handles[i], gymapi.IMAGE_DEPTH, dc_clients[i].get_depth_image_save_path(lift=False)) # Need fix, choose the right camera
            ...
            if dc_clients[i].state == "save lift visual data":
                ...
                if dc_clients[i].frame_count == 2:
                    rospy.loginfo("**Current state: " + dc_clients[i].state)
                    gym.write_camera_image_to_file(sim, envs[i], visual_cam_handles[i], gymapi.IMAGE_COLOR, dc_clients[i].get_rgb_image_save_path(lift=True)) # Need fix, choose the right camera
                    gym.write_camera_image_to_file(sim, envs[i], visual_cam_handles[i], gymapi.IMAGE_DEPTH, dc_clients[i].get_depth_image_save_path(lift=True)) # Need fix, choose the right camera  
                    # Save RGB image into the right folder (/suc_grasps or /fail_grasps)
                    gym.write_camera_image_to_file(sim, envs[i], visual_cam_handles[i], gymapi.IMAGE_COLOR, dc_clients[i].get_rgb_image_save_path_with_label()) # Need fix, choose the right camera                  
