...
if __name__ == "__main__":
    ...
    box_x = 0.2 #np.random.uniform(object_span + 0.1*amazon_box_scale/2 + 0.05, 0.3) * np.random.choice([-1, 1])
    box_y = 0   #np.random.uniform(-0.1, 0.1)
    ...
    visualization = False
    output_file = f"/home/baothach/Downloads/test_cam_views_meow.png" 
    ...
    while (not close_viewer) and (not all_done): 
        ...
        if state == "get shape servo plan":
            ...
            if action_count == 0:
                obj_angle = -np.pi/4   #np.random.uniform(-np.pi/2, np.pi/2)
            ...
            if action_count == 0:
                ...
                # gym.set_actor_dof_velocity_targets(robot_1.env_handle, robot_1.robot_handle, zero_velocity_action)
                # gym.set_actor_dof_velocity_targets(robot_2.env_handle, robot_2.robot_handle, zero_velocity_action)
                # state = "meoww"                 
